...
class PyCondition: public SGA::Condition {
  public:
   ...
   bool isFullfiled(
      const SGA::GameState& state, const std::vector< SGA::ActionTarget >& targets) const override
   {
...
class PySamplingMethod: public SGA::SamplingMethod {
  public:
   ...
   /* Trampoline (need one for each virtual function) */
   std::vector< SGA::Vector2i > getPositions(
      const SGA::GameState& gameState, const SGA::Vector2f& position) const override
   {
      PYBIND11_OVERRIDE_PURE_NAME(
   ...
   std::vector< int > getEntities(
      const SGA::GameState& gameState,
      const SGA::Vector2f& position,
      const std::unordered_set< int >& entityTypeIDs) const override
   {
      PYBIND11_OVERRIDE_PURE_NAME(
...
class PyEffect: public SGA::Effect {
  public:
   ...
   void execute(
      SGA::GameState& state,
      const SGA::ForwardModel& fm,
      const std::vector< SGA::ActionTarget >& targets) const override
   {
...
class PyAgent: public SGA::Agent {
  public:
   ...
   /* Trampoline (need one for each virtual function) */
   SGA::ActionAssignment computeAction(
      SGA::GameState state, const SGA::ForwardModel& forwardModel, SGA::Timer timer) override
   {
      PYBIND11_OVERRIDE_PURE_NAME(
   ...
   {
      PYBIND11_OVERRIDE_PURE_NAME(void, SGA::Agent, "init", init, state, &forwardModel, timer);
   }
};
...
PYBIND11_MODULE(stratega, m)
{
   ...
   py::bind_vector< std::vector<
      std::pair< SGA::TargetType, std::vector< std::shared_ptr< SGA::Condition > > > > >(
      m, "ActionTargetsList");
   ...
   py::bind_map< std::unordered_map< SGA::ParameterID, SGA::Parameter > >(
      m, "PlayerParameterTypesMap");
   py::bind_map< std::unordered_map< std::string, double > >(m, "ParameterValueMap");

   // Entities
   py::bind_map< std::unordered_map< std::string, std::unordered_set< int > > >(
      m, "EntityGroupsMap");
   ...
   // Categories
   py::bind_map< std::unordered_map< SGA::ActionCategory, std::vector< int > > >(
      m, "ActionCategoriesMap");
   py::bind_map< std::unordered_map< SGA::EntityCategory, std::vector< int > > >(
      m, "EntityCategoriesMap");

   py::class_< std::vector< SGA::Action > >(m, "ActionList")
      .def(py::init<>())
      .def(
         "__copy__",
         [](const std::vector< SGA::Action >& self) { return std::vector< SGA::Action >(self); })
      .def(
         "__deepcopy__",
         [](const std::vector< SGA::Action >& self, py::dict) {
            return std::vector< SGA::Action >(self);
         })
      .def("clear", &std::vector< SGA::Action >::clear)
      .def("__getitem__", [](const std::vector< SGA::Action >& v, int index) { return v[index]; })
      ...
      .def(
         "__iter__",
         [](std::vector< SGA::Action >& v) { return py::make_iterator(v.begin(), v.end()); },
         py::keep_alive< 0, 1 >());

   // ---- Vector2f ----
   py::class_< SGA::Vector2f >(m, "Vector2f")
      .def("__copy__", [](const SGA::Vector2f& self) { return SGA::Vector2f(self); })
      .def("__deepcopy__", [](const SGA::Vector2f& self, py::dict) { return SGA::Vector2f(self); })
      .def(py::init< double, double >(), py::arg("x") = 0.0, py::arg("y") = 0.0)
      .def_readwrite("x", &SGA::Vector2f::x)
      .def_readwrite("y", &SGA::Vector2f::y)
      .def("__repr__", [](const SGA::Vector2f& v) {
         return "(" + std::to_string(v.x) + "," + std::to_string(v.y) + ")";
      });

   // ---- Vector2i ----
   py::class_< SGA::Vector2i >(m, "Vector2i")
      .def("__copy__", [](const SGA::Vector2i& self) { return SGA::Vector2i(self); })
      .def("__deepcopy__", [](const SGA::Vector2i& self, py::dict) { return SGA::Vector2i(self); })
      .def(py::init< int, int >(), py::arg("x") = 0.0, py::arg("y") = 0.0)
      .def_readwrite("x", &SGA::Vector2i::x)
      .def_readwrite("y", &SGA::Vector2i::y)
      .def("__repr__", [](const SGA::Vector2i& v) {
         return "(" + std::to_string(v.x) + "," + std::to_string(v.y) + ")";
      });

   // ---- Action type ----
   py::class_< SGA::ActionType, std::shared_ptr< SGA::ActionType > >(
      m,
      "ActionType",
      "Contains the definition of an action. Which type is the source SGA::ActionSourceType , a "
      "set effects,preconditions and a list of action targets linked to their target conditions.If "
      "the action is continuous it will have a set of effects as events:OnStart, OnTick, "
      "OnComplete, OnAbort and the list of conditions that triggers the completed action.")
      .def(py::init<>())
      .def(
         "get_target_conditions",
         &SGA::ActionType::getTargetConditions,
         py::arg("searchingTarget"),
      ...
      .def(
         "get_source_type",
         &SGA::ActionType::getSourceType,
         "Source of this action (entity, player...)")
      .def(
         "set_source_type",
         &SGA::ActionType::setSourceType,
         py::arg("ast"),
         "Source of this action (entity, player...)")
      ...
      .def(
         "get_preconditions",
         [](SGA::ActionType& actionType) { return actionType.getPreconditions(); },
         "List of preconditions for this action to be run. These are tier 1 conditions: if they "
         "can't be fulfilled the action is not possible. If the conditions are true, the action "
         "may be possible based on situational circumstances.  (Example: having a resource to "
         "create an entity but no space in the board to spawn it).")
      .def(
         "get_targets",
         [](SGA::ActionType& actionType) { return actionType.getTargets(); },
         "List of target types of this action with their conditions. Each target is a pair "
         "<target, list<conditions>>. These are tier 2 conditions (i.e. situational), to be "
         "checked during action generation.")
      .def(
         "get_trigger_complete",
         [](SGA::ActionType& actionType) { return actionType.getTriggerComplete(); },
         "Continuous actions finish when certain conditions are met. This vector indicates those "
         "conditions.")
      .def(
         "get_effects",
         [](SGA::ActionType& actionType) { return actionType.getEffects(); },
         " List of effects this action causes when executed. This list is populated only for non "
         "continuous actions.")
      .def(
         "get_on_start",
         [](SGA::ActionType& actionType) { return actionType.getOnStart(); },
         "For continuous actions, list of effects that are applied when this action starts.")
      .def(
         "get_on_tick",
         [](SGA::ActionType& actionType) { return actionType.getOnTick(); },
         "For continuous actions, list of effects that are applied on every tick/turn. This effect "
         "is *not* applied on the tick this action starts, but it *will* also be applied in the "
         "tick it ends.")
      .def(
         "get_on_complete",
         [](SGA::ActionType& actionType) { return actionType.getOnComplete(); },
         "For continuous actions, list of effects that are applied when this action ends.")
      .def(
         "get_on_abort",
         [](SGA::ActionType& actionType) { return actionType.getOnAbort(); },
         "For continuous actions, list of effects that are applied when this action is aborted. "
         "Aborted actions do not execute the OnTick effect in the game tick they have been "
         "aborted.");

   // ---- Entity type ----
   py::class_< SGA::EntityType, std::shared_ptr< SGA::EntityType > >(
      m,
      "EntityType",
      "Contains the definition of the entity type. This information includes a list of parameters "
      "definitions, the actions that can execute, required technologies to spawn a entity, a list "
      "of spawnable entity types and a list of cost assigned to this entity type.")
      .def(py::init<>())

      .def(
         "get_parameter",
         &SGA::EntityType::getParameter,
         py::arg("id"),
         "Returns a SGA::Parameter reference that contains the parameter defintion")
      .def(
         "can_execute_action",
         &SGA::EntityType::canExecuteAction,
         py::arg("actionTypeID"),
         "Checks if this entity type is allowed to execute an action type")
      .def(
         "instantiate_entity",
         &SGA::EntityType::instantiateEntity,
         py::arg("entityID"),
         "Generate a new empty instance of this entity type ")
      .def(
         "get_param_max",
         &SGA::EntityType::getParamMax,
         py::arg("paramName"),
         "Returns the maximum value of a given parameter")
      .def(
         "get_param_min",
         &SGA::EntityType::getParamMin,
         py::arg("paramName"),
      ...
      .def(
         "get_symbol",
         &SGA::EntityType::getSymbol,
         "Symbol for this entity used in YAML for level placement.")
      .def(
         "set_symbol",
         &SGA::EntityType::setSymbol,
         py::arg("s"),
         "Symbol for this entity used in YAML for level placement.")

      .def_property(
         "required_tech_id",
         &SGA::EntityType::getRequiredTechID,
         &SGA::EntityType::setRequiredTechID)
      .def(
         "get_required_tech_id",
         &SGA::EntityType::getRequiredTechID,
         "ID of the technology that needs to be researched so this entity can be "
         "spawned/built/created in the game.")
      .def(
         "set_required_tech_id",
         &SGA::EntityType::setRequiredTechID,
         py::arg("id"),
         "ID of the technology that needs to be researched so this entity can be "
         "spawned/built/created in the game.")
      ...
      .def_property(
         "continuous_action_time",
         &SGA::EntityType::getContinuousActionTime,
         &SGA::EntityType::setContinuousActionTime)
      .def(
         "get_continuous_action_time",
         &SGA::EntityType::getContinuousActionTime,
         "Continuous Action Time")
      .def(
         "set_continuous_action_time",
         &SGA::EntityType::setContinuousActionTime,
         py::arg("d"),
         "Continuous Action Time")

      .def_property(
         "line_of_sight_range", &SGA::EntityType::getLoSRange, &SGA::EntityType::setLoSRange)
      .def(
         "get_line_of_sight_range",
         &SGA::EntityType::getLoSRange,
         "Range of the line of sight of this unit.")
      .def(
         "set_line_of_sight_range",
         &SGA::EntityType::setLoSRange,
         py::arg("d"),
         "Range of the line of sight of this unit.")

      .def(
         "get_parameters",
         [](SGA::EntityType& entityType) { return entityType.getParameters(); },
         "Map of parameters of this entity. Key is an integer (ID of the parameter) and value is "
         "the paramter object. The parameter defines the min, max and default value this parameter "
         "can take.")

      .def(
         "get_action_ids",
         [](SGA::EntityType& entityType) { return entityType.getActionIDs(); },
         "List of action IDs this entity can execute.")
      .def(
         "get_spawnable_entity_types",
         [](SGA::EntityType& entityType) { return entityType.getSpawnableEntityTypes(); },
         "For continuous actions, list of effects that are applied when this action is aborted. "
         "Aborted actions do not execute the OnTick effect in the game tick they have been "
         "aborted.")
      .def(
         "get_costs",
         [](SGA::EntityType& entityType) { return entityType.getCosts(); },
   ...
   py::class_< SGA::Player >(m, "Player")
      .def("__copy__", [](const SGA::Player& self) { return SGA::Player(self); })
      .def("__deepcopy__", [](const SGA::Player& self, py::dict) { return SGA::Player(self); })

      .def(py::init< int, bool >(), py::arg("id"), py::arg("canPlay"))

      .def(
         "can_execute_action",
         &SGA::Player::canExecuteAction,
         py::arg("actionTypeID"),
         "Indicates if a given action type can be executed by this player.")
      .def(
         "get_action_info",
         &SGA::Player::getActionInfo,
         py::arg("actionTypeID"),
         " Returns the ActionInfo of an action type this player can execute.")

      .def(
         "get_neutral_player_id",
         &SGA::Player::getNeutralPlayerID,
      ...
      .def(
         "set_can_play",
         &SGA::Player::setCanPlay,
         py::arg("can"),
         "Sets if the player can play now or not.")
      .def(
         "get_parameters",
         [](SGA::Player& player) { return player.getParameters(); },
         "Returns the list of parameters, can't be modified.")
      .def(
         "get_parameter",
         [](SGA::Player& player, int paramIdx) { return player.getParameter(paramIdx); },
         "Returns a const value of a parameter of this player.")

      .def(
         "set_parameter",
         &SGA::Player::setParameter,
         py::arg("paramIdx"),
         py::arg("val"),
         " Sets the parameter of this player to a certain value")
      .def(
         "resize_parameters",
         &SGA::Player::resizeParameters,
         py::arg("cap"),
         " Sets a size for the vector of parameters of this player.")
      .def(
         "remove_continuous_action",
         &SGA::Player::removeContinuousAction,
         py::arg("idx"),
         " Sets a size for the vector of parameters of this player.")
      .def(
         "add_continuous_action",
         &SGA::Player::addContinuousAction,
         py::arg("newAction"),
         " Adds a continuous action to the list of this player.")
      .def(
         "advance_continuous_action",
         &SGA::Player::advanceContinuousAction,
         py::arg("idx"),
         "Advances the tick counter of the continuous action with index 'idx'")
      .def(
         "get_continuous_actions",
         &SGA::Player::getContinuousActions,
         "Returns the list of continuous actions this player is executing.")

      .def(
         "get_attached_actions",
         &SGA::Player::getAttachedActions,
         " Returns the list of attached actions to this player.")
      .def(
         "get_attached_action",
         &SGA::Player::getAttachedAction,
         py::arg("idx"),
         " Returns the action info of the attached action with index 'idx'.")

      .def(
         "add_attached_action",
         &SGA::Player::addAttachedAction,
         py::arg("actionTypeID"),
         py::arg("lastExecutedTick"),
         "Adds a new attached action to this player.")
      .def(
         "set_action_last_tick",
         &SGA::Player::setActionLastTick,
         py::arg("idx"),
         py::arg("lastTick"),
         "Sets the last tick on an attached action, indexed by 'idx'")

      .def(
         "res_attached_actions",
         &SGA::Player::resAttachedActions,
         py::arg("cap"),
         "Reserves space for attached actions.");

   // ---- Entity ----
   py::class_< SGA::Entity >(
      m,
      "Entity",
      "Is an object that interacts with the game and responds to player input or other entities. "
      "This entity can be assigned to a player performing a set of actions per tick/turn. It has a "
      "list of parameters that store real values. If the player wants to check the SGA::EntityType "
      "it can search it using the assigned typeID.")
      .def("__copy__", [](const SGA::Entity& self) { return SGA::Entity(self); })
      .def("__deepcopy__", [](const SGA::Entity& self, py::dict) { return SGA::Entity(self); })
      .def(py::init<>())
      .def(
         "init",
         &SGA::Entity::init,
         py::arg("type"),
         py::arg("entityID"),
         "Initializes the entity with a given ID. Creates default attached actions and parameters "
         "list from the entity type.")
   ...
   py::class_< SGA::GameDescription, std::shared_ptr< SGA::GameDescription > >(m, "GameDescription")
      .def(
         ...
         &SGA::GameDescription::getActionCategories,
         "Returns the action categories of this game.")
      .def(
         "get_entity_categories",
         &SGA::GameDescription::getEntityCategories,
         "Returns the entity categories of this game.")
      .def(
         "get_action_types_ids",
         &SGA::GameDescription::getActionTypesIDs,
         py::arg("category"),
         "Returns all the actiontypes IDs of the selected action category.")
      .def(
         "get_entity_types_ids",
         &SGA::GameDescription::getEntityTypesIDs,
         py::arg("category"),
         "Returns all the entitytypes IDs of the selected action category.")
      .def(
         "get_action_types",
         &SGA::GameDescription::getActionTypes,
         py::arg("category"),
         py::arg("gameInfo"),
         "Returns all the actiontypes IDs of the selected action category.")
      .def(
         "get_entity_types",
         &SGA::GameDescription::getEntityTypes,
         py::arg("category"),
         py::arg("gameInfo"),
         "Returns all the entitytypes IDs of the selected action category.")
      .def(
         "is_from_category",
         &SGA::GameDescription::isFromCategory,
         py::arg("category"),
         py::arg("entityTypeId"),
         "Checks if a given entity type ID belongs to a given category")

      .def_static(
         "to_string", py::overload_cast< SGA::ActionCategory >(&SGA::GameDescription::toString))
      .def_static(
         "to_string", py::overload_cast< SGA::EntityCategory >(&SGA::GameDescription::toString));
   ...
   // ---- ActionCategory ----
   py::enum_< SGA::ActionCategory >(
      m,
      "ActionCategory",
      "Represents a grouping of action types into categories. Can be used by the agent to query "
      "SGA::ActionType grouped in different categories.")
   ...
   // ---- GameState ----
   py::class_< SGA::GameState >(
      m,
      "GameState",
      "Contains the game data without any logic, offering access to the current board, a list of "
      "player and their units.  If the agent want access to the definition of entity types, "
      "actions or game config yaml  it should access to SGA::GameInfo")
      ...
      .def("__copy__", [](const SGA::GameState& self) { return SGA::GameState(self); })
      .def(
         "__deepcopy__", [](const SGA::GameState& self, py::dict) { return SGA::GameState(self); })

      // Other
      .def(
         "can_execute_action",
         py::overload_cast< const SGA::Entity&, const SGA::ActionType& >(
            &SGA::GameState::canExecuteAction, py::const_),
         "Checks if a Entity can execute a given actionType")
      .def(
         "can_execute_action",
         py::overload_cast< const SGA::Player&, const SGA::ActionType& >(
            &SGA::GameState::canExecuteAction, py::const_),
         "Checks if a Player can execute a given actionType")
      .def(
         "get_player_action_types",
         &SGA::GameState::getPlayerActionTypes,
         py::arg("playerID"),
      ...
      .def(
         "is_researched",
         &SGA::GameState::isResearched,
         py::arg("playerID"),
         py::arg("technologyID"),
         "Check if technology is researched")
      .def(
         "can_research",
         &SGA::GameState::canResearch,
         py::arg("playerID"),
         py::arg("technologyID"),
         "Check if player can research an technology")
      .def(
         "research_technology",
         &SGA::GameState::researchTechnology,
         py::arg("playerID"),
         py::arg("technologyID"),
         "Research technology")
      .def(
         "init_research_techs",
         &SGA::GameState::initResearchTechs,
         "Initializes the research technologies to all players, to none.")

      ////Grid
      .def(
         "is_walkable",
         &SGA::GameState::isWalkable,
         py::arg("position"),
         "Checks if tile is occupied or the tile is walkable")
      .def(
         "is_in_bounds",
         py::overload_cast< const SGA::Vector2f& >(&SGA::GameState::isInBounds, py::const_),
         "Checks if position is inside of the tile map")
      .def(
         "is_in_bounds",
         py::overload_cast< const SGA::Vector2i& >(&SGA::GameState::isInBounds, py::const_),
         "Checks if position is inside of the tile map")

      .def(
         "initBoard",
         &SGA::GameState::initBoard,
         py::arg("boardWidth"),
         py::arg("tiles"),
      ...
      .def(
         "get_tile_at",
         py::overload_cast< int, int >(&SGA::GameState::getTileAt, py::const_),
         "Returns the tile at the position indicated in the parameter. Can throw an exception if "
         "out of bounds.")
      .def(
         "get_tile_at",
         py::overload_cast< const SGA::Vector2i& >(&SGA::GameState::getTileAt, py::const_),
         "Returns the tile at the position (x,y) indicated in the parameter. Can throw an "
         "exception if out of bounds.")

      ////Entities
      .def(
         "get_entity",
         py::overload_cast< int >(&SGA::GameState::getEntity),
         py::return_value_policy::reference,
         "Get entity")
      .def(
         "get_entity",
         py::overload_cast< SGA::Vector2f, float >(&SGA::GameState::getEntity),
         py::return_value_policy::reference,
         "Get entity")
      .def(
         "get_entity",
      ...
      .def(
         "get_entity_at",
         &SGA::GameState::getEntityAt,
         py::arg("pos"),
         py::return_value_policy::reference,
         "Returns an entity at board position 'pos'. It'll return a nullptr if no entities at this "
         "position. ")
      .def(
         "get_entity_const",
         &SGA::GameState::getEntityConst,
         py::arg("entityID"),
         py::return_value_policy::reference,
         "Returns an entity at board position 'pos'. It'll return a nullptr if no entities at this "
         "position. ")
      .def(
         "add_entity",
         &SGA::GameState::addEntity,
         py::arg("type"),
         py::arg("playerID"),
         py::arg("position"),
         "Adds a new entity of a given type to the game, in a given position, belonging to a "
         "specific player. ")

      .def(
         "get_player_entities",
         &SGA::GameState::getPlayerEntities,
         py::arg("playerID"),
         py::arg("entityCategory") = SGA::EntityCategory::Null,
         "Gets the list of entities of the specified player.")
      .def(
         "get_non_player_entities",
         &SGA::GameState::getNonPlayerEntities,
         py::arg("playerID"),
         py::arg("entityCategory") = SGA::EntityCategory::Null,
         "Gets the list of entities of the specified player.")
      .def("get_entities", [](SGA::GameState& state) { return state.getEntities(); })
      ////Player
      .def(
         "get_player",
         [](SGA::GameState& state, int id) { return state.getPlayer(id); },
         py::return_value_policy::reference,
         "Get player")
      .def(
         "who_can_play",
         &SGA::GameState::whoCanPlay,
         "Returns a list with the ID of players that can play in this game state.")
      .def(
         "can_play",
         &SGA::GameState::canPlay,
         py::arg("playerID"),
         "Indicates if the player with the provided ID can play in this game state.")
      .def(
         "get_players",
         [](SGA::GameState& state) { return state.getPlayers(); },
         py::return_value_policy::reference,
         "Get player")
      .def(
         "get_num_players",
         &SGA::GameState::getNumPlayers,
         "Gets the number of players in this game state.")
      .def("add_player", &SGA::GameState::addPlayer, py::arg("p"), "Adds player to gamestate")
      .def(
         "get_player_parameter",
         &SGA::GameState::getPlayerParameter,
         py::arg("playerID"),
         py::arg("paramName"),
         "Gets the value of a player parameter. Be sure to check first is the parameter you are "
         "asking for exist using SGA::GameState::hasPlayerParameter()")
      .def(
         "has_player_parameter",
         &SGA::GameState::hasPlayerParameter,
         py::arg("paramName"),
         "Indicates if the player has a specific parameter")
      .def(
         "get_player_parameter_names",
         &SGA::GameState::hasPlayerParameter,
         py::arg("playerID"),
         "Returns a list will all the parameter names of the player of which ID is given")
      .def(
         "get_player_parameters",
         &SGA::GameState::getPlayerParameters,
         py::arg("playerID"),
         "Gets a map with all pairs <parameter,value>")
      .def(
         "get_player_score",
         &SGA::GameState::getPlayerScore,
         py::arg("playerID"),
         "Returns the score of the player whose ID is passed.")

      .def(
         "apply_fog_of_war",
         &SGA::GameState::applyFogOfWar,
         py::arg("playerID"),
         "Removes entities and hide tiles that are not visible from the point of view of the given "
         "player.")
      .def(
         "get_fog_of_war_tile_id",
         &SGA::GameState::getFogOfWarTileId,
         "Returns the ID of the tile that represents the fog of war.")

      .def(
         "get_current_tbs_player",
         &SGA::GameState::getCurrentTBSPlayer,
         "Returns the ID of the player that moves in this state for Turn Based Games. For non-TBS, "
         "this value is -1.")
      .def(
         "set_current_tbs_player",
         &SGA::GameState::setCurrentTBSPlayer,
         py::arg("playerID"),
      ...
      .def(
         "set_game_type",
         &SGA::GameState::setGameType,
         py::arg("gt"),
         "Sets the type of game (enum type GameType)")
      .def(
         "get_game_type", &SGA::GameState::getGameType, "Returns the type of the game, of GameType")

      // Print information
      .def(
         "print_state_info",
         &SGA::GameState::printStateInfo,
         "Print all the entities of the current state")
      .def(
         "print_board",
         py::overload_cast< int >(&SGA::GameState::printBoard, py::const_),
         "Print view of the map of the current state")
      .def(
         "print_board",
         py::overload_cast<>(&SGA::GameState::printBoard, py::const_),
         "Print view of the map of the current state applying fog")
      .def(
         "print_entity_info",
         &SGA::GameState::printEntityInfo,
         py::arg("entityID"),
         "Print information of a specific entity")
      .def(
         "print_action_info",
         &SGA::GameState::printActionInfo,
         py::arg("action"),
   ...
   py::class_< SGA::Tile >(m, "Tile")
      .def(
         py::init< const SGA::TileType*, int, int >(),
         py::arg("tileType"),
         py::arg("x"),
         py::arg("y"))
   ...
   py::class_< SGA::Parameter >(m, "Parameter")
      ...
      .def_property(
         "default_value", &SGA::Parameter::getDefaultValue, &SGA::Parameter::setDefaultValue)
   ...
   // ---- ActionInfo ----
   py::class_< SGA::ActionInfo >(
      m,
      "ActionInfo",
      "Contains the last tick this action was executed and the action type ID. Used by forward "
      "models to check if the SGA::Entity or player can execute the action again after the "
      "cooldown.")
      .def(py::init<>())
   ...
   py::class_< SGA::Action >(m, "Action")
      .def("__copy__", [](const SGA::Action& self) { return SGA::Action(self); })
      .def("__deepcopy__", [](const SGA::Action& self, py::dict) { return SGA::Action(self); })
      ...
      .def(
         ...
         &SGA::Action::validate,
         py::arg("state"),
         "Checks if this action can be executed. It verifies if the conditions defined in the "
         "SGA::ActionType linked to this action are passed. This method also checks if the last "
         "time of the action execution is higher than the cooldown, and if all the preconditions "
         "and target conditions are fullfilled. It also checks that all the actions targets are "
         "valid.")
      .def(
         "is_entity_action",
         &SGA::Action::isEntityAction,
         "Checks if this action is to be executed over an entity.")
      .def(
         "is_player_action",
         &SGA::Action::isPlayerAction,
         "Checks if this action is to be executed over a player.")
      .def(
         "get_source_id",
         &SGA::Action::getSourceID,
         "Gets the player or entity ID of the target of this action")
      .def(
         "get_action_type_id",
         &SGA::Action::getActionTypeID,
         "Returns the action type ID of this action")
      .def("get_action_name", &SGA::Action::getActionName, "Returns the name of this action")
      .def(
         "get_action_type",
         &SGA::Action::getActionType,
         "Checks if this action is to be executed over an entity.")
      .def(
         "get_action_source_type",
         &SGA::Action::getActionSourceType,
         "Checks if this action is to be executed over an entity.")

      .def_static(
         "create_end_action",
         &SGA::Action::createEndAction,
         py::arg("playerID"),
         "Generates an Action used by the game to end the tick/turn.")
      .def_static(
         "create_abort_action",
         py::overload_cast< int, int, int >(&SGA::Action::createAbortAction),
         "Generates an Action which the owner is a entity, used by the game to abort a continuous "
         "action.")
      .def_static(
         "create_abort_action",
         py::overload_cast< int, int >(&SGA::Action::createAbortAction),
      ...
      .def_property(
         ...
         &SGA::Action::getContinuousActionID,
         &SGA::Action::setContinuousActionID)
      ...
      .def("get_targets", [](SGA::Action& action) { return action.getTargets(); })
   ...
   py::class_< SGA::TargetType >(m, "TargetType")
      ...
      .def(
         "is_valid",
         &SGA::TargetType::isValid,
         py::arg("state"),
         py::arg("actionTarget"),
         py::arg("sourceActionTarget"))
      ...
      .def_property(
         "sampling_method",
         &SGA::TargetType::getSamplingMethod,
         &SGA::TargetType::setSamplingMethod)
      ...
      .def_property(
         "group_entity_types",
         &SGA::TargetType::getGroupEntityTypes,
         &SGA::TargetType::setGroupEntityTypes)
      ...
      .def("get_technology_types", [](SGA::TargetType& targetType) {
         return targetType.getTechnologyTypes();
      });

   // ---- SamplingMethod ----
   py::class_< SGA::SamplingMethod, PySamplingMethod, std::shared_ptr< SGA::SamplingMethod > >(
      m, "SamplingMethod")
      .def(py::init<>())
      .def_readwrite("name", &SGA::SamplingMethod::name)
      .def(
         "get_positions",
         py::overload_cast< const SGA::GameState&, const SGA::Vector2f& >(
            &SGA::SamplingMethod::getPositions, py::const_))
      .def(
         "get_positions",
         py::overload_cast< const SGA::GameState& >(&SGA::SamplingMethod::getPositions, py::const_))
      .def(
         "get_entities",
         py::overload_cast<
            const SGA::GameState&,
            const SGA::Vector2f&,
            const std::unordered_set< int >& >(&SGA::SamplingMethod::getEntities, py::const_))
      .def(
         "get_entities",
         py::overload_cast< const SGA::GameState&, const std::unordered_set< int >& >(
            &SGA::SamplingMethod::getEntities, py::const_))
      .def(
         "validate_position",
         py::overload_cast< const SGA::GameState&, const SGA::Vector2f&, const SGA::Vector2f& >(
            &SGA::SamplingMethod::validatePosition, py::const_))
      .def(
         "validate_position",
         py::overload_cast< const SGA::GameState&, const SGA::Vector2f& >(
            &SGA::SamplingMethod::validatePosition, py::const_));
   ...
   py::class_< SGA::ActionTarget >(m, "ActionTarget")
      .def_static(
         "create_position_action_target",
         &SGA::ActionTarget::createPositionActionTarget,
         py::arg("position"))
      .def_static(
         "create_entity_action_target",
         &SGA::ActionTarget::createEntityActionTarget,
         py::arg("entityID"))
      .def_static(
         "create_player_action_target",
         &SGA::ActionTarget::createPlayerActionTarget,
         py::arg("playerID"))
      .def_static(
         "create_technology_entity_action_target",
         &SGA::ActionTarget::createTechnologyEntityActionTarget,
         py::arg("technologyID"))
      .def_static(
         "create_continuous_action_target",
         &SGA::ActionTarget::createContinuousActionActionTarget,
         py::arg("continuousActionID"))
      .def_static(
         "create_entity_type_action_target",
         &SGA::ActionTarget::createEntityTypeActionTarget,
         py::arg("entityTypeID"))
      .def_static(
         "create_tile_type_action_target",
         &SGA::ActionTarget::createTileTypeActionTarget,
         py::arg("entityTypeID"))
      ...
      .def(
         "get_player_id",
         py::overload_cast< const SGA::GameState& >(&SGA::ActionTarget::getPlayerID, py::const_))
      ...
      .def_static(
         "is_valid_with_targets",
         &SGA::ActionTarget::isValidWithTargets,
         py::arg("state"),
         py::arg("actionType"),
         py::arg("actionTargets"))
   ...
   py::class_< SGA::LevelDefinition >(m, "LevelDefinition")
      .def(
         py::init<
            std::vector< SGA::EntityPlacement >,
            SGA::Grid2D< std::shared_ptr< SGA::TileType > > >(),
         py::arg("entityPlacements"),
         py::arg("board"))
   ...
   // ---- TechnologyTreeCollection ----
   py::class_< SGA::TechnologyTreeCollection, std::shared_ptr< SGA::TechnologyTreeCollection > >(
      m, "TechnologyTreeCollection")
      .def_readwrite(
         "technology_tree_types",
         &SGA::TechnologyTreeCollection::technologyTreeTypes,
         "List of technology trees, indexed by a unique ID.")
      .def(
         "get_technology",
         &SGA::TechnologyTreeCollection::getTechnology,
         py::arg("technologyID"),
         "Returns a technology node with an ID corresponding to the one passed by parameter. The "
         "node could come from any of the trees of this collection.")
      .def(
         "get_technology_type_id",
         &SGA::TechnologyTreeCollection::getTechnologyTypeID,
         py::arg("technologyName"),
         "Returns the ID of a technology that matches the requested name, in any of the trees of "
         "this collection.");
   // ---- TechnologyTreeType ----
   py::class_< SGA::TechnologyTreeType, std::shared_ptr< SGA::TechnologyTreeType > >(
      m, "TechnologyTreeType")
      .def_readwrite(
         "technology_tree_name",
         &SGA::TechnologyTreeType::technologyTreeName,
         "Name of this technology tree")
      .def_readwrite(
         "technologies",
         &SGA::TechnologyTreeType::technologies,
         "Data of this tech tree, each entry contains and ID as key and a technology node as a "
         "value")
      .def(
         "get_technology_node",
         &SGA::TechnologyTreeType::getTechnologyNode,
         py::arg("technologyID"),
         "Returns a tech tree node from this tree with a corresponding technology ID.")
      .def(
         "exists_technology_tree_node",
         &SGA::TechnologyTreeType::existsTechnologyTreeNode,
         py::arg("technologyID"),
         "Checks if a given technology ID exists in this tree.")
      .def(
         "getTechnology_node_id",
         &SGA::TechnologyTreeType::getTechnologyNodeID,
         py::arg("technologyName"),
         "Returns the ID of a technology that matches the requested name.")
      .def(
         "exists_technology_node",
         &SGA::TechnologyTreeType::existsTechnologyNode,
         py::arg("technologyName"),
         "Checks if a technology with a given name exists in this tree.");
   // ---- TechnologyTreeNode ----
   py::class_< SGA::TechnologyTreeNode, std::shared_ptr< SGA::TechnologyTreeNode > >(
      m, "TechnologyTreeNode")
      .def_readwrite(
         "name", &SGA::TechnologyTreeNode::name, "Name of this technology as given in YAML")
      .def_readwrite(
         "description",
         &SGA::TechnologyTreeNode::description,
         "Description of this technology (provided in YAML)")
      .def_readwrite("id", &SGA::TechnologyTreeNode::id, "Unique ID of this technology.")
      .def_readwrite(
         "parent_ids",
         &SGA::TechnologyTreeNode::parentIDs,
         "IDs of the technologies that are required for this one to be researchable.")
      .def_readwrite(
         "cost",
         &SGA::TechnologyTreeNode::cost,
         "List of pairs (param,cost) that can be spend to research this technology.")
      .def_readwrite(
         "continuousActionTime",
         &SGA::TechnologyTreeNode::continuousActionTime,
         "How many ticks/turns takes to research this technology.")
      .def(
         "to_string",
         &SGA::TechnologyTreeNode::toString,
         py::arg("gameInfo"),
         "Returns a string with printable information of this research technology");
   // ---- Grid2D<Tile> ----
   py::class_< SGA::Grid2D< SGA::Tile > >(m, "BoardTiles")
      .def(
         py::init< int, int, SGA::Tile >(),
         py::arg("width"),
         py::arg("height"),
         py::arg("default_tile"))
      ...
      .def(
         "is_in_bounds",
         py::overload_cast< const SGA::Vector2i& >(
            &SGA::Grid2D< SGA::Tile >::isInBounds, py::const_))
      .def(
         "is_in_bounds",
         py::overload_cast< int, int >(&SGA::Grid2D< SGA::Tile >::isInBounds, py::const_));
   // ---- Grid2D<TileType> ----
   py::class_< SGA::Grid2D< std::shared_ptr< SGA::TileType > > >(m, "BoardTileTypes")
      .def(
         py::init< int, int, std::shared_ptr< SGA::TileType > >(),
         py::arg("width"),
         py::arg("height"),
         py::arg("default_tile"))
      ...
      .def(
         "get",
         [](const SGA::Grid2D< std::shared_ptr< SGA::TileType > >& b, int x, int y) {
            return b.get(x, y);
         })
      ...
      .def(
         "is_in_bounds",
         py::overload_cast< const SGA::Vector2i& >(
            &SGA::Grid2D< std::shared_ptr< SGA::TileType > >::isInBounds, py::const_))
      .def(
         "is_in_bounds",
   ...
   py::class_< SGA::GameRunner >(m, "GameRunner")
      .def(
         ...
         [](SGA::GameRunner& a,
            ...
            // Set seed of the agents for deterministic behaviour - ToDo Should we move this into
            // Stratega & Should it be done automatically with generateAgents?
         ...
         [](SGA::GameRunner& a, py::list agents, SGA::Vector2f& resolution, int seed = 0) {
            ...
            for(auto& agent : agents) {
               if(pybind11::str(agent, true).check()) {
                  newAgents.emplace_back(
                     SGA::AgentFactory::get().createAgent(agent.cast< std::string >()));
               } else {
      ...
      .def(
         "reset",
         py::overload_cast< int >(&SGA::GameRunner::reset),
         py::arg("levelID"),
         "Resets the game to an initial state.")
      .def(
         "reset",
         py::overload_cast<>(&SGA::GameRunner::reset),
         "Resets the game to an initial state with a specific map.")
      .def(
         "step",
         &SGA::GameRunner::step,
         py::arg("actions"),
         "Advances the game by one timestep. When the game has ended, you are responsible for "
         "calling GameRunner::reset() to reset the environments state.")
      .def(
         "render",
         &SGA::GameRunner::render,
         "Renders a visual representation of the game. May create a new window when called for the "
         "first time.")
      .def(
         "get_gamestate",
         &SGA::GameRunner::getGameState,
   ...
   py::class_< SGA::GameConfig >(m, "GameConfig")
      ...
      .def_property_readonly(
         "forward_model",
         [](const SGA::GameConfig& config) { return config.forwardModel.get(); },
      ...
      .def("get_forward_model", [](SGA::GameConfig* a) { return std::move(a->forwardModel); })
      // Methods
   ...
   py::class_< SGA::GameInfo, std::shared_ptr< SGA::GameInfo > >(m, "GameInfo")
      ...
      .def(
         "get_entity_type",
         &SGA::GameInfo::getEntityType,
         py::arg("entityTypeID"),
         "Returns the entity type.")
      .def(
         "get_tile_type",
         &SGA::GameInfo::getTileType,
         py::arg("tileTypeID"),
         "Returns the tile type.")

      .def(
         "get_parameter_global_id",
         &SGA::GameInfo::getParameterGlobalID,
         py::arg("parameterName"),
         "Returns the ID of the parameter.")
      .def(
         "get_parameter_type",
         &SGA::GameInfo::getParameterType,
         py::arg("entityTypeID"),
         py::arg("globalParameterID"),
         "Returns the parameter type from a entityType.")
      .def(
         "check_entity_have_parameter",
         &SGA::GameInfo::checkEntityHaveParameter,
         py::arg("entityTypeID"),
         py::arg("parameterName"),
         "Returns true if the entity type has the searched parameter")
      .def(
         "get_player_parameter",
         &SGA::GameInfo::getPlayerParameter,
         py::arg("id"),
         "Returns the parameter of the player.")

      .def(
         "get_action_type_const",
         &SGA::GameInfo::getActionTypeConst,
         py::arg("actionTypeID"),
         "Returns the action type.")
      .def(
         "get_action_type",
         &SGA::GameInfo::getActionType,
         py::arg("typeID"),
         "Returns the action type.")
      .def(
         "get_action_type_id",
         &SGA::GameInfo::getActionTypeID,
         py::arg("actionName"),
         "Returns the action type ID.")

      .def(
         "get_technology_counts",
         &SGA::GameInfo::getTechnologyCounts,
         "Returns a map of tuples with the tree ID and how many technologies each has.")
      .def(
         "get_technology",
         &SGA::GameInfo::getTechnology,
         py::arg("technologyID"),
         "Returns the action type.")
      .def(
         "get_technology_type_id",
         &SGA::GameInfo::getTechnologyTypeID,
         py::arg("technologyName"),
         "Returns the ID of a technology that matches the requested name, in any of the trees of "
         "the tree collection.")
      .def(
         "get_tree_nodes",
         &SGA::GameInfo::getTreeNodes,
         py::arg("techTreeID"),
         "Returns a list with all technologies of a given tree, specified by its ID")
      .def(
         "get_tech_trees_ids",
         &SGA::GameInfo::getTechTreesIDs,
         "Returns the IDs of the tech trees of this game.")

      .def(
         "get_game_description",
         &SGA::GameInfo::getGameDescription,
      ...
      .def(
         "set_technology_tree_collection",
         &SGA::GameInfo::setTechnologyTreeCollection,
         py::arg("tt"))
   ...
   py::class_< SGA::MinimizeDistanceHeuristic >(m, "MinimizeDistanceHeuristic")
      ...
      .def(
         "evaluate_gamestate",
         &SGA::MinimizeDistanceHeuristic::evaluateGameState,
         py::arg("forwardModel"),
         py::arg("gameState"),
         py::arg("playerID"));
   ...
   py::class_< SGA::ActionAssignment >(m, "ActionAssignment")
      .def(
         "__copy__", [](const SGA::ActionAssignment& self) { return SGA::ActionAssignment(self); })
      .def(
         "__deepcopy__",
         [](const SGA::ActionAssignment& self, py::dict) { return SGA::ActionAssignment(self); })
      .def(py::init<>())
      .def(
         "assign_action_or_replace",
         &SGA::ActionAssignment::assignActionOrReplace,
         py::arg("newAction"),
         "Assigns the action to the corresponding entity or player. Any existing assignment will "
         "be overwritten.")
      .def(
         "merge",
         &SGA::ActionAssignment::merge,
         py::arg("merge"),
      ...
      .def(
         "get_entity_action",
         &SGA::ActionAssignment::getEntityAction,
         py::arg("entityID"),
         " Returns the assignment for an entity.")
      .def(
         "get_player_action",
         &SGA::ActionAssignment::getPlayerAction,
         py::arg("playerID"),
         "Returns the assignment for an player.")
      .def(
         "get_assignment_count",
         &SGA::ActionAssignment::getAssignmentCount,
      ...
      .def_static(
         "from_single_action",
         &SGA::ActionAssignment::fromSingleAction,
         py::arg("a"),
         "Constructs a ActionAssignment from a single action, useful for TBS-Games.")
      .def_static(
         "create_end_action_assignment",
         &SGA::ActionAssignment::createEndActionAssignment,
         py::arg("playerID"),
   ...
   py::class_< SGA::ForwardModel >(m, "EntityForwardModel")
      .def(
         "can_player_lost",
         &SGA::ForwardModel::checkPlayerLost,
         py::arg("state"),
         py::arg("player"),
         "Checks if a player has lost the game due to the game's lose conditions.")
      .def(
         "check_player_won",
         &SGA::ForwardModel::checkPlayerWon,
         py::arg("state"),
         py::arg("player"),
         "Returns if a player won the game by attending to the winning conditions defined in the "
         "rules.")
      .def(
         "spawn_entity",
         &SGA::ForwardModel::spawnEntity,
         py::arg("state"),
         py::arg("entityType"),
         py::arg("playerID"),
         py::arg("position"),
         "Spawns an entity in the game with the default spawn entity method. A list of On Spawn "
         "effects are  executed just after spawning the entity")

      .def(
         "add_win_conditions",
         &SGA::ForwardModel::addWinConditions,
         py::arg("conditions"),
         " Adds a list of conditions for the game to be won for a player.")
      .def(
         "add_lose_conditions",
         &SGA::ForwardModel::addLoseConditions,
         py::arg("conditions"),
         "Adds a list of conditions for the game to be lost for a player.")
      .def(
         "get_win_conditions",
         &SGA::ForwardModel::getWinConditions,
         "Returns a list of sub-lists with all WIN conditions. Each sub-list contains a group of "
         "conditions that must be fulfilled for the game to be over for a player. The game will be "
         "WON by a player if all conditions in a sub-list are fullfiled, for at least one of the "
         "sub-lists returned. ")
      .def(
         "get_lose_conditions",
         &SGA::ForwardModel::getLoseConditions,
         "Returns a list of sub-lists with all LOSE conditions. Each sub-list contains a group of "
         "conditions  that must be fulfilled for the game to be over for a player. The game will "
         "be LOST by a player if all  conditions in a sub-list are fullfiled, for at least one of "
         "the sub-lists returned. ")

      .def(
         "add_on_tick_effect",
         &SGA::ForwardModel::addOnTickEffect,
         py::arg("ote"),
         "Adds an OnTickEffect to the forward mode, which will be executed every game tick.")
      .def(
         "add_on_entity_spawn_effect",
         &SGA::ForwardModel::addOnEntitySpawnEffect,
         py::arg("ose"),
         "Adds an OnEntitySpawnEffect to the forward mode, which will be executed every time an "
         "entity is spawned.")

      .def(
         "get_on_tick_effects",
         &SGA::ForwardModel::getOnTickEffects,
         "Returns all effects that are exxecuted on every tick of the game.")
      .def(
         "get_on_entity_spawn_effects",
         &SGA::ForwardModel::getOnEntitySpawnEffects,
   ...
   py::class_< SGA::TBSForwardModel, SGA::ForwardModel >(m, "TBSForwardModel")
      ...
      .def(
         "generate_actions",
         py::overload_cast< const SGA::GameState&, int >(
            &SGA::ForwardModel::generateActions, py::const_),
         " Generates actions in the given gamestate by the received player and fills the action "
         "vector passed by parameter.")
      .def(
         "generate_actions",
         py::overload_cast< const SGA::GameState&, int, std::vector< SGA::Action >& >(
            &SGA::ForwardModel::generateActions, py::const_),
         "Returns a list of available actions in the given gamestate by the received player")

      .def(
         "advance_gamestate",
         py::overload_cast< SGA::GameState&, const SGA::Action& >(
            &SGA::TBSForwardModel::advanceGameState, py::const_),
         "Executes an action in a given SGA::GameState before updating the entities of the "
         "gamestate that should be removed and checking if the game is over.")
      .def(
         "advance_gamestate",
         py::overload_cast< SGA::GameState&, const SGA::ActionAssignment& >(
            &SGA::TBSForwardModel::advanceGameState, py::const_),
         "Executes a list of actions.")

      .def(
         "end_turn",
         &SGA::TBSForwardModel::endTurn,
         py::arg("state"),
         "End the turn of the current player and if all the player has played it ends the current "
         "game turn.")
      .def(
         "check_game_is_finished",
         &SGA::TBSForwardModel::checkGameIsFinished,
         py::arg("state"),
   ...
   py::class_< SGA::RTSForwardModel, SGA::ForwardModel >(m, "RTSForwardModel")
      ...
      .def(
         "advance_gamestate",
         py::overload_cast< SGA::GameState&, const SGA::Action& >(
            &SGA::RTSForwardModel::advanceGameState, py::const_),
         "Moves all the entities and resolves collisions before and after executing an action in a "
         "given Gamestate")
      .def(
         "advance_gamestate",
         py::overload_cast< SGA::GameState&, const SGA::ActionAssignment& >(
      ...
      .def(
         "move_entities",
         &SGA::RTSForwardModel::moveEntities,
         py::arg("state"),
         "Moves all the entities that have a current path and they did not reach their "
         "destination. If the entity has a path it moves the entity through all the path points "
         "one after theother until reaching the last one.")
      .def(
         "resolve_entity_collisions",
         &SGA::RTSForwardModel::resolveEntityCollisions,
         py::arg("state"),
         "Resolves collisions between entities in a basic way computing the penetration depth and "
         "pushing them way in the opposite direction.")
      .def(
         "resolve_environment_collisions",
         &SGA::RTSForwardModel::resolveEnvironmentCollisions,
         py::arg("state"),
      ...
      .def(
         "check_game_is_finished",
         &SGA::RTSForwardModel::checkGameIsFinished,
         py::arg("state"),
         "Checks if the game is finished by current limit or because a player has won.");
   ...
   py::class_< SGA::Agent, PyAgent, std::shared_ptr< SGA::Agent > /* <--- trampoline*/ >(m, "Agent")
      ...
      .def(
         "computeAction",
         &SGA::Agent::computeAction,
         "Function for deciding the next action to execute. Must be overriden for an agent to "
         "work. Returns an ActionAssignment")
      .def(
         "init",
         &SGA::Agent::init,
         "Function for initializing the agent. Override this function to receive a call just "
         "before starts.")
   ...
   py::class_< SGA::Timer >(m, "Timer")
      ...
      .def(
         "set_max_time_milliseconds", &SGA::Timer::setMaxTimeMilliseconds, py::arg("newMaxTimeMs"))
