...
{
   auto iter = std::find_if(std::begin(entities), std::end(entities), [&](Entity const& p) {
...
std::vector< Entity > GameState::getPlayerEntities(
   int playerID, EntityCategory entityCategory) const
{
   ...
   for(auto& entity : entities) {
      if(entity.getOwnerID() == playerID) {
         // Either no category was especified (default argment) or the entity type id belongs to
         // this category.
         if(entityCategory == EntityCategory::Null
            || gameInfo->getGameDescription().isFromCategory(
               entityCategory, entity.getEntityTypeID())) {
...
std::vector< Entity > GameState::getNonPlayerEntities(
   int playerID, EntityCategory entityCategory) const
{
   std::vector< Entity > ret;
   for(auto& entity : entities) {
...
double GameState::getPlayerParameter(int playerID, const std::string& paramName) const
{
   ...
   if(p != nullptr) {
      ...
      throw std::runtime_error(
         "No parameter " + paramName + " associated to player ID " + std::to_string(playerID));
...
void GameState::applyFogOfWar(int playerID)
{
   ...
   for(auto entity : entities) {
      if(entity.getOwnerID() == playerID) {
         ...
         auto leftX = std::max< int >(
            0, static_cast< int >(entity.x() - entity.getLineOfSightRange()));
         auto rightX = std::min< int >(
            static_cast< int >(board.getWidth() - 1),
            static_cast< int >(entity.x() + entity.getLineOfSightRange()));
         auto leftY = std::max< int >(
            0, static_cast< int >(entity.y() - entity.getLineOfSightRange()));
         auto rightY = std::min< int >(
            static_cast< int >(board.getHeight() - 1),
            static_cast< int >(entity.y() + entity.getLineOfSightRange()));
...
bool GameState::canExecuteAction(const Entity& entity, const ActionType& actionType) const
{
   ...
   for(const auto& precondition : actionType.getPreconditions()) {
      if(! precondition->isFullfiled(
            *this, {ActionTarget::createEntityActionTarget(entity.getID())})) {
         return false;
      }
   }
...
bool GameState::canExecuteAction(const Player& player, const ActionType& actionType) const
{
   ...
   for(const auto& precondition : actionType.getPreconditions()) {
      if(! precondition->isFullfiled(
            *this, {ActionTarget::createPlayerActionTarget(player.getID())})) {
         return false;
      }
   }
...
const Entity* GameState::getEntityAt(const Vector2f& pos) const
{
   for(const auto& entity : entities) {
      if(static_cast< int >(pos.x) == static_cast< int >(entity.x())
         && static_cast< int >(pos.y) == static_cast< int >(entity.y())) {
         return &entity;
      }
   }
...
bool GameState::canResearch(int playerID, int technologyID) const
{
   ...
   const TechnologyTreeNode& technologyNode = gameInfo->getTechnologyTreeCollection().getTechnology(
      technologyID);

   const std::vector< int >& parentsIDs = technologyNode.parentIDs;

   for(auto& parent : parentsIDs) {
      const TechnologyTreeNode& technologyParentNode = gameInfo->getTechnologyTreeCollection()
                                                          .getTechnology(parent);
...
const Tile& GameState::getTileAt(const Vector2i& pos) const
{
   if(isInBounds(pos))
      return board[pos];
   throw std::runtime_error(
      "Access to board out of bounds: " + std::to_string(pos.x) + "," + std::to_string(pos.y));
}
...
void GameState::printStateInfo() const
{
   ...
   for(auto& entity : entities) {
      ...
      std::cout << "	[type]: " << gameInfo->getEntityType(entity.getEntityTypeID()).getName()
                << " [entityID]: " << entity.getID() << std::endl;
   }
}
...
void GameState::printActionInfo(const Action& action) const
{
   ...
   if(action.getActionFlag() == ActionFlag::AbortContinuousAction) {
      ...
      for(size_t i = 0; i < actionType.getTargets().size(); i++) {
         std::cout << "(Target: " << actionType.getTargets()[i].first.getTypeString() << ", "
