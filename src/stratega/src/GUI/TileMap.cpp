...
namespace SGA {
void TileMap::init(
   const GameState& state, const GameConfig& gameConfig, const RenderConfig& renderConfig)
{
   // Initialise atlas
...
void TileMap::update(
   const GameState& state, const GameState& fowState, bool renderFow, FogRenderType fogType)
{
      ...
      for(int y = 0; y < state.getBoardHeight(); ++y) {
         ...
         if(! renderFow) {
            ...
            updateTileQuad(
               quad,
               tile,
               fogTile.getTileTypeID() == -1 ? sf::Color::Transparent : sf::Color::White);
         } else if(fogType == FogRenderType::Fog) {
            updateTileQuad(quad, fogTile, sf::Color::White);
         } else if(fogType == FogRenderType::Tiles) {
            updateTileQuad(
               quad, tile, fogTile.getTileTypeID() == -1 ? FOG_COLOR : sf::Color::White);
         }
      }
   }
}

void TileMap::updateTileQuad(
   sf::Vertex* quadPtr, const Tile& tile, const sf::Color& quadColor) const
{
