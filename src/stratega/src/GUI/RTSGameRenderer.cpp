...
RTSGameRenderer::RTSGameRenderer(SGA::Vector2f& newResolution)
    : state(),
      ...
      config(nullptr),
      window(
         sf::VideoMode((int) newResolution.x, (int) newResolution.y),
         "Stratega GUI",
   ...
   while(window.pollEvent(event)) {
      ...
      switch(event.type) {
         ...
         case sf::Event::MouseWheelScrolled: {
            mouseScrolled(event, view, window);
            break;
         }
         case sf::Event::MouseButtonReleased: {
            mouseButtonReleased(event, view, window);
            break;
         }
         case sf::Event::MouseButtonPressed: {
            mouseButtonPressed(event, view, window);
            break;
         }
         case sf::Event::MouseMoved: { /* Ignore mouse movement unless a button is pressed*/
            mouseMoved(event, view, window);
            break;
         }
         case sf::Event::KeyPressed: {
            keyPressed(event, view, window);
            break;
         }
         default: break;
      }
...
void RTSGameRenderer::mouseScrolled(
   const sf::Event& event, sf::View& view, sf::RenderWindow& /*window*/)
{
...
void RTSGameRenderer::mouseButtonReleased(
   const sf::Event& event, sf::View& /*view*/, sf::RenderWindow& /*window*/)
{
   ...
   if(event.mouseButton.button == sf::Mouse::Left) {
      ...
      auto movementDistance = std::sqrt(
         movementDir.x * movementDir.x + movementDir.y * movementDir.y);
      ...
      if(movementDistance <= 10.0) {
         ...
         for(auto& unit : state.getEntities()) {
            ...
            if(screenPos.x > xLeft && screenPos.x < xRight && screenPos.y > yLeft
               && screenPos.y < yRight) {
...
void RTSGameRenderer::mouseButtonPressed(
   const sf::Event& event, sf::View& /*view*/, sf::RenderWindow& newWindow)
{
   ...
   if(event.mouseButton.button == sf::Mouse::Middle) {
      moving = true;
      oldMousePosition = newWindow.mapPixelToCoords(
         sf::Vector2i(event.mouseButton.x, event.mouseButton.y));
   }
}
void RTSGameRenderer::mouseMoved(
   const sf::Event& event, sf::View& view, sf::RenderWindow& newWindow)
{
   // Determine the new position in world coordinates
   const sf::Vector2f newPos = newWindow.mapPixelToCoords(
...
void RTSGameRenderer::keyPressed(
   const sf::Event& /*event*/, sf::View& view, sf::RenderWindow& newWindow)
{
   // Camera movement
...
void RTSGameRenderer::renderLayers()
{
   ...
   for(auto i = actionsSettings.selectedEntities.begin();
       i != actionsSettings.selectedEntities.end();) {
   ...
   for(const auto& unit : selectedGameStateCopy->getEntities()) {
      if(actionsSettings.isSelected(unit.getID())) {
         ...
         selectionSprite.setOrigin(
            selectionTexture.getSize().x / 2.f, selectionTexture.getSize().y / 2.f);
         selectionSprite.setPosition(
            toISO(unit.x(), unit.y()) + sf::Vector2f{TILE_WIDTH_HALF, TILE_HEIGHT_HALF});
   ...
   if(drawDebug) {
      ...
      if(mesh) {
         ...
         for(int i = 0; i < selectedGameStateCopy->getRTSNavigation()->m_navMesh->getMaxTiles();
             ++i) {
   ...
   for(auto& unit : selectedGameStateCopy->getEntities()) {
      ...
      if(unit.getPath().m_nstraightPath > 0) {
         ...
         for(int i = 0; i < unit.getPath().m_nstraightPath - 1; ++i) {
            auto v1 = toISO(
               unit.getPath().m_straightPath[i * 3], unit.getPath().m_straightPath[i * 3 + 2]);
            auto v2 = toISO(
               unit.getPath().m_straightPath[(i + 1) * 3],
               unit.getPath().m_straightPath[(i + 1) * 3 + 2]);

            // Adjust to sprites offset
            v1.x += TILE_WIDTH_HALF;
            v2.x += TILE_WIDTH_HALF;

            sf::Vertex line[] = {sf::Vertex(v1), sf::Vertex(v2)};
         ...
         // temp.setPosition(toISO(unit.executingAction.targetPosition.x,
         // unit.executingAction.targetPosition.y)); temp.move(TILE_WIDTH_HALF, 0);
...
void RTSGameRenderer::createBottomBar()
{
   ...
   ImGui::SetNextWindowSize(
      ImVec2(static_cast< float >((float) window.getSize().x), (float) window.getSize().y / 4));
...
void RTSGameRenderer::createWindowUnits()
{
   ...
   for(auto& unit : state.getEntities()) {
      ...
      std::string unitInfo;
      unitInfo = type.getName() + " " + std::to_string(unit.getID())
                 + " PID: " + std::to_string(unit.getOwnerID());
...
void RTSGameRenderer::createWindowActionList()
{
   ...
   int index = 0;
   if(pointOfViewPlayerID != NO_PLAYER_ID) {
      auto actionsHumanCanPlay = config->forwardModel->generateActions(
         fowState, pointOfViewPlayerID);
            ...
            for(auto& targetType : action.getTargets()) {
               switch(targetType.getType()) {
                  case ActionTarget::Position:
                     actionInfo += " x:" + std::to_string((int) targetType.getPosition(state).x)
                                   + ",y:" + std::to_string((int) targetType.getPosition(state).y);
                     break;
                  case ActionTarget::EntityReference:
                     actionInfo += state.getEntity(targetType.getEntityID())
                                      ->getEntityType()
                                      .getName();
                     break;
   ...
   if(pointOfViewPlayerID != NO_PLAYER_ID) {
      ...
      for(const auto& parameter : state.getGameInfo()->getPlayerParameterTypes()) {
         ...
         stream << std::fixed << std::setprecision(2)
                << player->getParameter(parameter.second.getIndex());
