...
std::vector< Action > getWidgetResult(GameState& state, ActionsSettings& settings, int playerID)
{
   ...
   if(settings.hasActionTypeSelected()) {
      // Selected actiontype
      const ActionType& actionType = state.getGameInfo()->getActionType(
         settings.actionTypeSelected);
   ...
   std::vector< Action >& actionsToExecute)
{
   if(settings.hasEntitiesSelected()) {
      int elementNumber = 0;
      for(auto& possibleTechnology :
          actionType.getTargets()[settings.selectedTargets.size()].first.getTechnologyTypes()) {
            ...
            for(const auto& action : state.getEntity(entityID)->getContinuousActions()) {
               if(action.getActionTypeID() == settings.actionTypeSelected) {
                  ...
                  if(ImGui::Button("Abort", ImVec2(50, 50))) {
                     actionsToExecute.emplace_back(Action::createAbortAction(
                        playerID,
                        state.getEntity(entityID)->getID(),
                        action.getContinuousActionID()));
                  }
                  ImGui::PopStyleColor(3);
         ...
         if(ImGui::Button(
               state.getGameInfo()
                  ...
                  .getTechnology(possibleTechnology)
                  .name.c_str(),
               ImVec2(50, 50))) {
            ...
            actionTargets.insert(
               actionTargets.end(),
               settings.selectedTargets.begin(),
               settings.selectedTargets.end());
            actionTargets.emplace_back(
               ActionTarget::createTechnologyEntityActionTarget(possibleTechnology));
            ...
            for(auto& entity : settings.selectedEntities) {
               ...
               for(auto& condition :
                   actionType.getTargets()[settings.selectedTargets.size()].second) {
            ...
            if(canExecute)
               settings.selectedTargets.emplace_back(
                  ActionTarget::createTechnologyEntityActionTarget(possibleTechnology));
         }
         if((elementNumber++ % 4) < 3)
            ImGui::SameLine();
         ImGui::PopID();
      }
   FINISH1:
      ...
      for(auto& possibleTechnology :
         ...
         for(auto& action : state.getPlayer(playerID)->getContinuousActions()) {
            if(action.getActionTypeID() == settings.actionTypeSelected) {
               ...
               ImGui::PushStyleColor(ImGuiCol_ButtonActive, (ImVec4) ImColor::HSV(0, 0.8f, 0.8f));
               if(ImGui::Button("Abort", ImVec2(50, 50))) {
                  actionsToExecute.emplace_back(
                     Action::createAbortAction(playerID, action.getContinuousActionID()));
               }
               ImGui::PopStyleColor(3);
...
void getEntityType(
   GameState& state, ActionsSettings& settings, int playerID, const ActionType& actionType)
{
   if(settings.hasEntitiesSelected()) {
      int elementNumber = 0;
      for(auto& possibleActionType :
          actionType.getTargets()[settings.selectedTargets.size()].first.getGroupEntityTypes()) {
         bool canSpawn = true;

         // Check if any selectedEntity can spawn the targets
         for(auto& entity : settings.selectedEntities) {
            auto& entityType = state.getEntity(entity)->getEntityType();
            if(entityType.getSpawnableEntityTypes().find(possibleActionType)
               == entityType.getSpawnableEntityTypes().end()) {
               canSpawn = false;
            }
         }
         ...
         ImGui::PushID(elementNumber);
         if(ImGui::Button(
               state.getGameInfo()->getEntityType(possibleActionType).getName().c_str(),
               ImVec2(50, 50))) {
            bool canExecute = true;
            ...
            actionTargets.insert(
               ...
               settings.selectedTargets.end());
            actionTargets.emplace_back(
               ActionTarget::createEntityTypeActionTarget(possibleActionType));
      ...
      for(auto& possibleActionType :
         ...
         if(state.getGameInfo()->getPlayerSpawnableTypes().find(possibleActionType)
            == state.getGameInfo()->getPlayerSpawnableTypes().end()) {
            canSpawn = false;
         }
...
void verifyPlayerActionTargets(
   GameState& state,
   ActionsSettings& settings,
   int playerID,
   std::vector< Action >& actionsToExecute,
   ...
   Action& newAction)
{
   ...
   actionTargets.emplace_back(ActionTarget::createPlayerActionTarget(playerID));
   actionTargets.insert(
      actionTargets.end(), settings.selectedTargets.begin(), settings.selectedTargets.end());
   ...
   if(player->canExecuteAction(settings.actionTypeSelected)) {
      if(state.getCurrentTick()
            - player->getActionInfo(settings.actionTypeSelected).lastExecutedTick
...
void verifyEntityActionTargets(
   GameState& state,
   ActionsSettings& settings,
   int playerID,
   std::vector< Action >& actionsToExecute,
   ...
   Action& newAction)
{
   ...
   for(auto& entityID : settings.selectedEntities) {
      ...
      if(state.getCurrentTick()
            - entity->getActionInfo(settings.actionTypeSelected).lastExecutedTick
         < actionType.getCooldown())
         continue;
...
void getActionType(GameState& state, ActionsSettings& settings, int playerID)
{
   ...
   for(auto& actionType : actionTypes) {
      ImGui::PushID(elementNumber);
      if(ImGui::Button(
            state.getGameInfo()->getActionType(actionType).getName().c_str(), ImVec2(50, 50))) {
...
void getPlayerPossibleActionTypes(
   GameState& state,
   ActionsSettings& /*settings*/,
   int playerID,
   std::unordered_set< int >& actionTypes)
...
void getEntityPossibleActionTypes(
   GameState& state,
   ActionsSettings& settings,
   int /*playerID*/,
   std::unordered_set< int >& actionTypes)
