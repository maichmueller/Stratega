...
namespace SGA {
EntityRenderer::EntityRenderer() : atlas(2) {}

void EntityRenderer::init(
   const GameState& initialState, const GameConfig& gameConfig, const RenderConfig& renderConfig)
{
   // Initialise atlas
...
void EntityRenderer::update(const GameState& state)
{
   ...
   for(auto entity = entities.begin(); entity != entities.end(); entity++) {
      ...
      auto start = toISO(entity->x(), entity->y())
                   - sf::Vector2f{spriteSize.x / 4.f, spriteSize.y / 1.4f};
      ...
      quadPtr[0].color = entity->isNeutral() ? sf::Color::Transparent
                                             : PLAYER_COLORS[entity->getOwnerID()];
      quadPtr[1].color = entity->isNeutral() ? sf::Color::Transparent
                                             : PLAYER_COLORS[entity->getOwnerID()];
      quadPtr[2].color = entity->isNeutral() ? sf::Color::Transparent
                                             : PLAYER_COLORS[entity->getOwnerID()];
      quadPtr[3].color = entity->isNeutral() ? sf::Color::Transparent
                                             : PLAYER_COLORS[entity->getOwnerID()];
      i++;
   }
}
