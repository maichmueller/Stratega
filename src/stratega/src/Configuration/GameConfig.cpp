...
std::unique_ptr< GameState > GameConfig::generateGameState(int levelID) const
{
   ...
   gameInfo.setPlayerParameterTypes(
      std::make_shared< std::unordered_map< ParameterID, Parameter > >(playerParameterTypes));
   ...
   gameInfo.setParameterIDLookup(
      std::make_shared< std::unordered_map< std::string, ParameterID > >(parameters));
   gameInfo.setTechnologyTreeCollection(
      std::make_shared< TechnologyTreeCollection >(technologyTreeCollection));
   gameInfo.setPlayerSpawnableTypes(
      std::make_shared< std::unordered_set< EntityTypeID > >(playerSpawnableTypes));
   gameInfo.setYAMLPath(yamlPath);
   gameInfo.setGameDescription(
      std::make_shared< GameDescription >(actionCategories, entityCategories));
