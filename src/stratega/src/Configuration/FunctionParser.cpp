...
std::optional< AbstractFunctionCall > FunctionParser::parseAbstractFunction(
   const std::string& code, const ParseContext& context) const
{
   // Remove whitespace
   auto copy = code;
   copy.erase(
      std::remove_if(copy.begin(), copy.end(), [](char x) { return std::isspace(x); }), copy.end());
   ...
   while(ss.peek() != ')' && ! ss.eof()) {
      ...
      if(((param = parseConstant(ss))) || ((param = parseEntityPlayerReference(ss, context)))
...
std::optional< FunctionParameter > FunctionParser::parseConstant(std::istringstream& ss) const
{
   ...
   if(std::isdigit(ss.peek())) {
      double value;
      ss >> value;
      return FunctionParameter::createConstParameter(
         static_cast< double >(prefix == '-' ? -value : value));
   }

   return {};
}

std::optional< FunctionParameter > FunctionParser::parseParameterReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
...
std::optional< FunctionParameter > FunctionParser::parseEntityPlayerReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   auto names = parseAccessorList(ss, 2);
...
std::optional< FunctionParameter > FunctionParser::parseTimeReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   auto names = parseAccessorList(ss, 2);
...
std::optional< FunctionParameter > FunctionParser::parseEntityPlayerParameterReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   ...
   if(targetIt == context.targetIDs.end() || parameterIt == context.parameterIDs.end()) {
      throw std::runtime_error(
         "Unknown parameter/action-target: " + targetName + ".Player." + parameterName);
   }

   return FunctionParameter::createEntityPlayerParameterReference(
      {parameterIt->second, targetIt->second});
}

std::optional< FunctionParameter > FunctionParser::parseTargetReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
...
std::optional< FunctionParameter > FunctionParser::parseEntityTypeReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   auto names = parseAccessorList(ss, 1);
...
std::optional< FunctionParameter > FunctionParser::parseTileTypeReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   auto names = parseAccessorList(ss, 1);
...
std::optional< FunctionParameter > FunctionParser::parseTechnologyTypeReference(
   std::istringstream& ss, const ParseContext& context) const
{
   auto begin = ss.tellg();
   auto names = parseAccessorList(ss, 1);
...
std::optional< std::vector< std::string > > FunctionParser::parseAccessorList(
   std::istringstream& ss, size_t length) const
{
   auto begin = ss.tellg();
