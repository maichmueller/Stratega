...
std::unordered_map< int, LevelDefinition > loadLevelsFromYAML(
   const std::string& fileMapsPath, const GameConfig& config)
{
   GameConfigParser parser;
   return parser.parseLevelsFromFile(fileMapsPath, config);
}

std::unordered_map< int, LevelDefinition > GameConfigParser::parseLevelsFromFile(
   const std::string& fileMapsPath, const GameConfig& config) const
{
...
std::unique_ptr< GameConfig > GameConfigParser::parseFromFile(const std::string& filePath) const
{
   ...
   // Order is important, only change if you are sure that a function doesn't depend on something
   // parsed before it
   ...
   for(auto& type : config->entityTypes) {
      // Assign actions to entities
      auto actions = types[type.second.getName()]["Actions"].as< std::vector< std::string > >(
      ...
      type.second.setSpawnableEntityTypes(
         parseEntityGroup(types[type.second.getName()]["CanSpawn"], *config));
      auto name = types[type.second.getName()]["RequiredTechnology"].as< std::string >("");
      type.second.setRequiredTechID(
         name.empty() ? TechnologyTreeType::UNDEFINED_TECHNOLOGY_ID
                      : config->technologyTreeCollection.getTechnologyTypeID(name));
      // Hardcoded cost information
      type.second.setCosts(parseCost(types[type.second.getName()]["Cost"], *config));
   }

   // Assign player actions
   auto actions = configNode["Player"]["Actions"].as< std::vector< std::string > >(
      std::vector< std::string >());
   for(const auto& actionName : actions) {
...
void GameConfigParser::parseBoardGenerator(const YAML::Node& boardNode, GameConfig& config) const
{
   ...
   if(boardNode["GenerationType"].as< std::string >() == "Manual") {
      ...
      throw std::runtime_error(
         "Unknown board-generation type " + boardNode["GenerationType"].as< std::string >());
   }
}
...
void GameConfigParser::parseEntityGroups(
   const YAML::Node& entityGroupsNode, GameConfig& config) const
{
   ...
   for(const auto& idEntityPair : config.entityTypes) {
      ...
      config.entityGroups.emplace(
         idEntityPair.second.getName(), std::initializer_list< int >{idEntityPair.first});
   }
}
...
void GameConfigParser::parseActions(const YAML::Node& actionsNode, GameConfig& config) const
{
   ...
   for(const auto& nameTypePair : actionsNode.as< std::map< std::string, YAML::Node > >()) {
      ...
      for(auto& target :
          nameTypePair.second["Targets"].as< std::map< std::string, YAML::Node > >()) {
         ...
         auto targetConditions = target.second["Conditions"].as< std::vector< std::string > >(
            std::vector< std::string >());
      ...
      // Parse preconditions
      auto preconditions = nameTypePair.second["Preconditions"].as< std::vector< std::string > >(
         std::vector< std::string >());
      parser.parseFunctions(preconditions, type.getPreconditions(), context);

      // Parse effects
      auto effects = nameTypePair.second["Effects"].as< std::vector< std::string > >(
         std::vector< std::string >());
      ...
      if(nameTypePair.second["TriggerComplete"].IsDefined()) {
         ...
         auto targetConditions = nameTypePair.second["TriggerComplete"]
                                    .as< std::vector< std::string > >(std::vector< std::string >());
      ...
      if(nameTypePair.second["OnStart"].IsDefined()) {
         ...
         auto effectStrings = nameTypePair.second["OnStart"].as< std::vector< std::string > >(
            std::vector< std::string >());
      ...
      if(nameTypePair.second["OnTick"].IsDefined()) {
         ...
         auto effectStrings = nameTypePair.second["OnTick"].as< std::vector< std::string > >(
            std::vector< std::string >());
      ...
      if(nameTypePair.second["OnComplete"].IsDefined()) {
         ...
         auto effectStrings = nameTypePair.second["OnComplete"].as< std::vector< std::string > >(
            std::vector< std::string >());
      ...
      if(nameTypePair.second["OnAbort"].IsDefined()) {
         ...
         auto effectStrings = nameTypePair.second["OnAbort"].as< std::vector< std::string > >(
            std::vector< std::string >());
...
TargetType GameConfigParser::parseTargetType(const YAML::Node& node, const GameConfig& config) const
{
   ...
   if(targetType.getType() == TargetType::Position) {
      targetType.setSamplingMethod(
         node["SamplingMethod"].as< std::shared_ptr< SamplingMethod > >());
      ...
      if(techNode.IsScalar() && techNode.as< std::string >() == "All") {
         ...
         auto technologies = techNode.as< std::vector< std::string > >(
            std::vector< std::string >());
         // Assigne technology IDs to the technologytypes map
         for(auto& technology : technologies) {
            targetType.getTechnologyTypes().insert(
               config.technologyTreeCollection.getTechnologyTypeID(technology));
         }
      }
   }
   return targetType;
...
ActionCategory GameConfigParser::parseActionCategory(const std::string& name) const
{
   ...
   if(name == "Attack")
      ...
      throw std::runtime_error("Cannot find action category: " + name);
...
EntityCategory GameConfigParser::parseEntityCategory(const std::string& name) const
{
   ...
   if(name == "Base")
      entityCategory = SGA::EntityCategory::Base;
   else if(name == "Building")
      entityCategory = SGA::EntityCategory::Building;
   else if(name == "Spawner")
      entityCategory = SGA::EntityCategory::Spawner;
   else if(name == "Unit")
      entityCategory = SGA::EntityCategory::Unit;
   else if(name == "NoFighter")
      entityCategory = SGA::EntityCategory::NoFighter;
   else if(name == "Fighter")
      entityCategory = SGA::EntityCategory::Fighter;
   else if(name == "Melee")
      entityCategory = SGA::EntityCategory::Melee;
   else if(name == "Ranged")
      entityCategory = SGA::EntityCategory::Ranged;
   else
      throw std::runtime_error("Cannot find entity category: " + name);
...
void GameConfigParser::parseForwardModel(const YAML::Node& fmNode, GameConfig& config) const
{
   ...
   auto winConditions = fmNode["WinConditions"].as< std::map< std::string, YAML::Node > >(
      std::map< std::string, YAML::Node >());

   for(auto& winCondition : winConditions) {
      std::vector< std::shared_ptr< Condition > > conditionList;

      auto conditions = winCondition.second.as< std::vector< std::string > >(
         std::vector< std::string >());
   ...
   auto loseConditions = fmNode["LoseConditions"].as< std::map< std::string, YAML::Node > >(
      std::map< std::string, YAML::Node >());

   for(auto& loseCondition : loseConditions) {
      std::vector< std::shared_ptr< Condition > > conditionList;

      auto conditions = loseCondition.second.as< std::vector< std::string > >(
         std::vector< std::string >());
      parser.parseFunctions< Condition >(conditions, conditionList, context);
   ...
   for(const auto& trigger :
       fmNode["Trigger"].as< std::vector< YAML::Node > >(std::vector< YAML::Node >())) {
      auto map = trigger.as< std::map< std::string, YAML::Node > >();
      if(map.find("OnTick") != map.end()) {
         auto nameEffectsPair = *map.find("OnTick");

         context.targetIDs.emplace("Source", 0);
         auto conditions = nameEffectsPair.second["Conditions"].as< std::vector< std::string > >(
            std::vector< std::string >());
         auto effects = nameEffectsPair.second["Effects"].as< std::vector< std::string > >(
            std::vector< std::string >());

         // Initiliaze OnTickEffect
         OnTickEffect onTickEffect;
         onTickEffect.validTargets = parseEntityGroup(
            nameEffectsPair.second["ValidTargets"], config);
         ...
         onSpawnEffect.validTargets = parseEntityGroup(
            nameEffectsPair.second["ValidTargets"], config);
         ...
         // std::cerr << "Unknown trigger with name " << nameEffectsPair.first << " will be
         // ignored." << std::endl;
      }
...
void GameConfigParser::parseTechnologyTrees(
   const YAML::Node& techtreeNode, GameConfig& config) const
{
   ...
   for(const auto& nameTypePair : types) {
      ...
      for(const auto& nameTechPair :
          nameTypePair.second.as< std::map< std::string, YAML::Node > >()) {
      ...
      config.technologyTreeCollection.technologyTreeTypes[static_cast< int >(
         config.technologyTreeCollection.technologyTreeTypes.size())] = technologyTreeType;
   }
   ...
   for(auto& technologyTreeType : config.technologyTreeCollection.technologyTreeTypes) {
      ...
      for(auto& technology : technologyTreeType.second.technologies) {
         ...
         auto techTreeTypeYaml = typeMap[technologyTreeType.second.technologyTreeName]
                                    .as< std::map< std::string, YAML::Node > >();
         // Find the technology
         auto technologyYaml = techTreeTypeYaml[technology.second.name]
                                  .as< std::map< std::string, YAML::Node > >();
         // Get the parents of the technology
         auto parentsNames = technologyYaml["Requirements"].as< std::vector< std::string > >(
            std::vector< std::string >());
...
void GameConfigParser::parseGameDescription(
   const YAML::Node& gameDescription, GameConfig& config) const
{
...
void GameConfigParser::parseGameRunner(const YAML::Node& gameRunner, GameConfig& config) const
{
   ...
   config.initDisqualificationBudgetTimeMs = agentInitializationTime["DisqualificationTimeMs"]
                                                .as< long >(60);
   ...
   config.disqualificationBudgetTimeMs = agentComputationTime["DisqualificationTimeMs"].as< long >(
      60);
...
void GameConfigParser::parseRenderConfig(const YAML::Node& configNode, GameConfig& config) const
{
   ...
   for(const auto& entityNode : configNode["Entities"]) {
      ...
      config.renderConfig->entitySpritePaths.emplace(
         entityName, parseFilePath(entityConfig["Sprite"], config));
   }
   ...
   for(const auto& tileNode : configNode["Tiles"]) {
      ...
      config.renderConfig->tileSpritePaths.emplace(
         tileName, parseFilePath(tileConfig["Sprite"], config));
   }
}
...
void GameConfigParser::parseParameterList(
   const YAML::Node& parameterNode,
   GameConfig& config,
   std::unordered_map< ParameterID, Parameter >& parameterBucket) const
{
   for(const auto& nameParamPair :
       parameterNode.as< std::map< std::string, double > >(std::map< std::string, double >())) {
...
std::unordered_set< EntityTypeID > GameConfigParser::parseEntityGroup(
   const YAML::Node& groupNode, const GameConfig& config) const
{
   ...
   if(groupNode.IsSequence()) {
      ...
      for(const auto& name : names) {
         ...
         std::copy(
            ...
            groupIt->second.end(),
            std::inserter(anonymousGroup, anonymousGroup.end()));
      }
...
std::unordered_map< ParameterID, double > GameConfigParser::parseCost(
   const YAML::Node& costNode, const GameConfig& config) const
{
   auto nameCostMap = costNode.as< std::map< std::string, double > >(
...
std::string GameConfigParser::parseFilePath(
   const YAML::Node& pathNode, const GameConfig& config) const
{
   try {
...
void GameConfigParser::parseMaps(
   const YAML::Node& mapsLayouts,
   std::unordered_map< int, LevelDefinition >& levelDefinitions,
...
void GameConfigParser::parseLevelDefinition(
   const YAML::Node& mapLayout,
   std::string mapName,
   std::unordered_map< int, LevelDefinition >& levelDefinitions,
   const GameConfig& config) const
{
   ...
   mapString.erase(
      std::remove_if(
         mapString.begin(), mapString.end(), [](char x) { return x != '\n' && std::isspace(x); }),
      mapString.end());
   ...
   for(size_t i = 0; i < mapString.size(); i++) {
      ...
      if(c == '\n') {
         ...
         if(width == -1) {
            ...
            throw std::runtime_error(
               "Line " + std::to_string(y) + " contains " + std::to_string(x)
               + " symbols. Expected " + std::to_string(width));
         }
      ...
      if(entityIt != entityLookup.end()) {
         ...
         if(i < mapString.size() - 1 && std::isdigit(mapString[i + 1])) {
            ownerID = static_cast< int >(
               mapString[i + 1] - '0');  // Convert char '0','1',... to the corresponding integer
            if(ownerID >= config.getNumberOfPlayers()) {
               throw std::runtime_error(
                  "Tried assigning the entity " + entityIt->second->getName()
                  + " to an unknown player " + std::to_string(ownerID));
            }
            i++;
         }
         ...
         throw std::runtime_error(
            "Encountered unknown symbol '" + std::string(1, c) + "'when parsing the board.");
      }
   ...
   if(mapString[mapString.size() - 1] != '\n') {
      ...
      if(x != width) {
         throw std::runtime_error(
            "Line " + std::to_string(y) + " contains " + std::to_string(x) + " symbols. Expected "
            + std::to_string(width));
      }
   }
   ...
   LevelDefinition newLevel(
      entityPlacements,
      Grid2D< std::shared_ptr< TileType > >(width, tileTypes.begin(), tileTypes.end()));
