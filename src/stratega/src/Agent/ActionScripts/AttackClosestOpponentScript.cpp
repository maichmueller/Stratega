...
Action AttackClosestOpponentScript::getAction(
   const GameState& gameState, std::vector< Action >& actionSpace, int playerID)
{
   if(actionSpace.size() > 1) {
      ...
      // prioritize attacks, if at least one attack action targeting an opponent entity is
      // available, return the attack action to the closest opponent
      ...
      if(not subActions.empty()) {
         std::optional<size_t> bestAction = std::nullopt;
         for(size_t i = 0; i < subActions.size(); i++) {
            auto& action = subActions.at(i);

            if(opponentUnits.contains(action.getTargets()[1].getEntityID())) {
               const double dist = positions[action.getTargets()[0].getEntityID()]
                                      .manhattanDistance(
                                         positions[action.getTargets()[1].getEntityID()]);
         ...
         if(bestAction.has_value()) {
            return subActions[bestAction.value()];
}
      }
      ...
      if(! subActions.empty()) {
         ...
         for(size_t i = 0; i < subActions.size(); i++) {
            auto& action = subActions.at(i);
            const double dist = minimalDistanceToOpponents(
               action.getTargets()[1].getPosition(gameState), positions, opponentUnits);
            if(dist < actionDistance) {
               actionDistance = dist;
      ...
      subActions = filterAllButThisActionType(
         actionSpace, "Attack", actionTypeIDToActionTypeString);
...
Action AttackClosestOpponentScript::getActionForUnit(
   const GameState& gameState, std::vector< Action >& actionSpace, int playerID, int unitID)
{
   ...
   if(! suitableActions.empty()) {
      ...
      // available, return the attack action to the closest opponent
      double actionDistance = std::numeric_limits< int >::max();
      auto subActions = filterActionTypes(
         suitableActions, "Attack", actionTypeIDToActionTypeString);
      if(! subActions.empty()) {
         int bestAction = -1;
         for(size_t i = 0; i < subActions.size(); i++) {
            auto& action = subActions.at(i);
            ...
            if(opponentUnits.contains(action.getTargets()[1].getEntityID())) {
               const double dist = action.getTargets()[0].getPosition(gameState).manhattanDistance(
                  positions[action.getTargets()[1].getEntityID()]);
               if(dist < actionDistance) {
                  actionDistance = dist;
                  bestAction = (int) i;
      ...
      if(! subActions.empty()) {
         ...
         for(size_t i = 0; i < subActions.size(); i++) {
            auto& action = subActions.at(i);
            const double dist = minimalDistanceToOpponents(
               positions[action.getTargets()[1].getEntityID()], positions, opponentUnits);
            if(dist < actionDistance) {
               actionDistance = dist;
               bestAction = static_cast< int >(i);
            }
...
double AttackClosestOpponentScript::minimalDistanceToOpponents(
