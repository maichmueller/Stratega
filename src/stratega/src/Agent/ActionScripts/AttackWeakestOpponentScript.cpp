...
double AttackWeakestOpponentScript::getHealth(
   const Entity& entity, const GameState& gamestate) const
{
   for(const auto& param :
       (gamestate.getGameInfo()->getEntityTypes()).at(entity.getEntityTypeID()).getParameters()) {
      if(param.second.getName() == "Health") {
         return entity.getParamValues()[static_cast< unsigned long >(param.second.getIndex())];
      }
   }
   return 0;
}

Action AttackWeakestOpponentScript::getAction(
   const GameState& gameState, std::vector< Action >& actionSpace, int playerID)
{
   if(actionSpace.size() > 1) {
      ...
      if(! subActions.empty()) {
         ...
         for(size_t i = 0; i < subActions.size(); i++) {
            auto& action = subActions.at(i);
            const double dist = action.getTargets()[1].getPosition(gameState).manhattanDistance(
               positions[weakestUnitID]);
...
Action AttackWeakestOpponentScript::getActionForUnit(
