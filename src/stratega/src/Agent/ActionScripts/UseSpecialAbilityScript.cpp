...
namespace SGA {
Action UseSpecialAbilityScript::getAction(
   const GameState& gameState, std::vector< Action >& actionSpace, int /*playerID*/)
{
   if(actionSpace.size() > 1) {
      ...
      auto subsetActions = filterAllButThisActionType(
         actionSpace, "Attack", actionTypeIDToActionTypeString);
      subsetActions = filterAllButThisActionType(
         subsetActions, "Move", actionTypeIDToActionTypeString);
      subsetActions = filterAllButThisActionType(
         subsetActions, "EndTurn", actionTypeIDToActionTypeString);
...
Action UseSpecialAbilityScript::getActionForUnit(
   ...
   int unitID) const
{
   ...
   if(suitableActions.size() > 1) {
      ...
      // if there is any special ability to use, return a random one, else default to any random
      // action
      if(! subsetActions.empty())
         return subsetActions[rand() % subsetActions.size()];
   }
