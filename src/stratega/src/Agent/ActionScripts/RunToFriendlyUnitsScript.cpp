...
namespace SGA {
Action RunToFriendlyUnitsScript::getAction(
   ...
   if(actionSpace.size() > 1) {
      ...
      if(! subActions.empty()) {
         ...
         for(size_t i = 0; i < subActions.size(); i++) {
            ...
            const int dist = (int) maximalDistanceToFriends(
               action.getTargets()[1].getPosition(gameState), positions, friendlyUnits);
...
Action RunToFriendlyUnitsScript::getActionForUnit(
...
double RunToFriendlyUnitsScript::maximalDistanceToFriends(
