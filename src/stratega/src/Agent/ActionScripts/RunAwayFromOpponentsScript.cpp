...
namespace SGA {
Action RunAwayFromOpponentScript::getAction(
   const GameState& gameState, std::vector< Action >& actionSpace, int playerID)
{
   if(actionSpace.size() > 1) {
      ...
      std::vector< Action > subActions = filterActionTypes(
         actionSpace, "Move", actionTypeIDToActionTypeString);
      if(! subActions.empty()) {
         ...
         for(size_t i = 0; i < subActions.size(); i++) {
            ...
            const int dist = (int) minimalDistanceToOpponents(
...
Action RunAwayFromOpponentScript::getActionForUnit(
   const GameState& gameState, std::vector< Action >& actionSpace, int playerID, int unitID) const
{
   ...
   if(suitableActions.size() > 1) {
      ...
      // else, try to move closer to an opponent
      std::vector< Action > subActions = filterActionTypes(
         suitableActions, "Move", actionTypeIDToActionTypeString);
      if(! subActions.empty()) {
         int actionDistance = std::numeric_limits< int >::min();
...
double RunAwayFromOpponentScript::minimalDistanceToOpponents(
