...
void MCTSNode::searchMCTS(
   ForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator)
{
   ...
   ForwardModel& forwardModel, MCTSParameters& params, std::mt19937& /*randomGenerator*/)
{
   ...
   children.push_back(std::unique_ptr< MCTSNode >(
      new MCTSNode(forwardModel, std::move(gsCopy), this, childIndex, this->ownerID)));
...
MCTSNode* MCTSNode::uct(MCTSParameters& params, std::mt19937& randomGenerator)
{
   ...
   // Keep the best action as decided by UCT. If I'm moving in this node, we'll aim to maximize the
   // UCT value. Otherwise, minimize.
   int which = -1;
   double bestValue = amIMoving ? -std::numeric_limits< double >::max()
   ...
   if(params.rolloutsEnabled) {
      ...
      while(! (rolloutFinished(gsCopy, thisDepth, params) || gsCopy.isGameOver())) {
         ...
         applyActionToGameState(
            forwardModel,
            gsCopy,
            actions.at(randomDistribution(randomGenerator)),
            params,
            playerID);
         thisDepth++;
      }

      // We evaluate the state at the end of the rollout using the heuristic specified in the
      // parameter settings. We then return this reward to the last node expanded in the tree.
      return normalize(
         params.heuristic->evaluateGameState(forwardModel, gsCopy, params.PLAYER_ID), 0, 1);
   }

   return normalize(
      params.heuristic->evaluateGameState(forwardModel, gameState, params.PLAYER_ID), 0, 1);
}
   ...
   int /*playerID*/) const
{
   ...
   // Continue rolling the state until the game is over, we run out of budget or this agent can play
   // again.
   while(! targetGameState.canPlay(params.PLAYER_ID) && ! params.isBudgetOver()
         && ! targetGameState.isGameOver()) {
      // Roll actions for the opponent(s).
      params.currentFMCalls += SGA::rollOppOnly(targetGameState, forwardModel, params);
   }
}

// Backpropagation in MCTS. Update number of visits, accummulated reward value and node reward
// bounds.
...
int MCTSNode::mostVisitedAction(MCTSParameters& params, std::mt19937& randomGenerator)
{
   ...
   for(size_t i = 0; i < children.size(); i++) {
      if(children[i] != nullptr) {
         ...
         childValue = noise(
            childValue, params.epsilon, params.doubleDistribution_(randomGenerator));
