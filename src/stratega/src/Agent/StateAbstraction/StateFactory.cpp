...
AbstractState StateFactory::createAbstractState(GameState& tbsState)
{
   ...
   std::vector< double > stateInfo = {
      static_cast< double >(tbsState.getCurrentTBSPlayer()),
      static_cast< double >(tbsState.getCurrentTick())};
   ...
   for(auto entry : config.insertEntityParameters) {
      if(entry.second == true) {
         ...
         for(auto entity : tbsState.getEntities()) {
            const auto& entityType = tbsState.getGameInfo()->getEntityType(
