...
namespace SGA {
ActionAssignment ActionAbstractionMCTSAgent::computeAction(
...
void ActionAbstractionMCTSAgent::init(
   GameState initialState, const ForwardModel& forwardModel, Timer timer)
{
   ...
   std::unique_ptr< BaseActionScript >
      attackClose = std::make_unique< AttackClosestOpponentScript >();
   parameters_.portfolio.emplace_back(std::move(attackClose));
   std::unique_ptr< BaseActionScript >
      attackWeak = std::make_unique< AttackWeakestOpponentScript >();
