...
namespace SGA {
ActionAssignment DFSAgent::computeAction(
...
double DFSAgent::evaluateRollout(
   const ForwardModel& forwardModel, GameState& gameState, int depth, const int playerID)
{
...
void DFSAgent::applyActionToGameState(
   const ForwardModel& forwardModel, GameState& gameState, const Action& action, int playerID)
{
   ...
   while(! gameState.canPlay(getPlayerID()) && remainingForwardModelCalls > 0
