...
TreeNode* TreeNode::expand(ForwardModel& forwardModel, AgentParameters& agentParameters)
{
   ...
   agentParameters.currentFMCalls += SGA::roll(
      gsCopy, forwardModel, action, playerID, agentParameters);

   children.emplace_back(std::unique_ptr< TreeNode >(new TreeNode(
      forwardModel, std::move(gsCopy), this, static_cast< int >(children.size()), this->ownerID)));
