...
namespace SGA {
ActionAssignment BFSAgent::computeAction(
   GameState state, const ForwardModel& forwardModel, Timer timer)
{
   ...
   } else  // else we run a full search
   {
      ...
      previousActionIndex = parameters_.continuePreviousSearch
                                  && (action.getActionFlag() == ActionFlag::EndTickAction)
                               ? bestActionIndex
                               : -1;
...
/// <summary>
/// In case the tree is reused, we fill the openNodes and knownLeaves list according to the selected
/// subtree.
/// </summary>
...
int BFSAgent::getBestActionIdx(ForwardModel& forwardModel)
{
   ...
   // all nodes in openNodes and knownLeaves represent the end of a search path and could be the
   // best node
   for(TreeNode* node : openNodes) {
      const double value = heuristic->evaluateGameState(
         forwardModel, node->gameState, parameters_.PLAYER_ID);
