...
namespace SGA {
ActionAssignment BeamSearchAgent::computeAction(
...
Action BeamSearchAgent::beamSearch(ForwardModel& forwardModel, TreeNode& root)
{
   ...
   for(size_t i = 1; i < parameters_.PLAYER_BEAM_DEPTH; i++) {
      ...
      if(newBestSimulations.size() > parameters_.PLAYER_BEAM_WIDTH)
         newBestSimulations.erase(
            newBestSimulations.begin() + parameters_.PLAYER_BEAM_WIDTH, newBestSimulations.end());
...
bool BeamSearchAgent::sortByValue(const TreeNode* i, const TreeNode* j)
{
...
std::vector< TreeNode* > BeamSearchAgent::simulate(ForwardModel& forwardModel, TreeNode& node)
{
   ...
   for(auto& i : node.children) {
      auto* child = i.get();
      child->value = parameters_.heuristic->evaluateGameState(
         forwardModel, child->gameState, parameters_.PLAYER_ID);
   ...
   // !!!Note: the index of the child is no longer associated with the index of an action in the
   // node's action space!!!
   if(bestSimulations.size() > parameters_.PLAYER_BEAM_WIDTH)
      bestSimulations.erase(
         bestSimulations.begin() + parameters_.PLAYER_BEAM_WIDTH, bestSimulations.end());
