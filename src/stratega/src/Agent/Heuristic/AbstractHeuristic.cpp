...
double AbstractHeuristic::evaluateGameState(
   const ForwardModel& /*forwardModel*/, GameState& gameState, int playerID)
{
   ...
   if(gameState.isGameOver()) {
      if(gameState.getWinnerID() == playerID)
         score += 1000;  // this score needs to be maximized. so winning gives you 1000 points, but
                         // in a fog of war game, this will never trigger
      else
         score -= 1000;
   }
   ...
   for(auto entry : attributeWeights) {
      ...
      if(parameterWeight != 0) {
         ...
         for(auto entity : gameState.getEntities()) {
            const auto& entityType = gameState.getGameInfo()->getEntityType(
            ...
            for(const auto& parameter : entityType.getParameters()) {
               if(parameter.second.getName() == parameterName) {
                  ...
                  if(den > 0.0) {
                     ...
                     // double URQValue = entity.parameters[parameter.second.index] /
