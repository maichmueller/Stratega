...
/// </summary>
RHEAGenome::RHEAGenome(
   const ForwardModel& forwardModel, GameState gameState, RHEAParameters& params)
{
   ...
   while(! gameState.isGameOver() && actionSpace.size() > 0 && length < params.individualLength) {
      // Until the end of the sequence: choose and apply a random action to the state with the
      // forward model.
...
void RHEAGenome::applyActionToGameState(
   ...
   RHEAParameters& params)
{
   ...
   // again.
   while(! gameState.canPlay(params.PLAYER_ID) && ! params.isBudgetOver() > 0
...
RHEAGenome RHEAGenome::crossover(
   const ForwardModel& forwardModel,
   GameState gameState,
   RHEAParameters& params,
   std::mt19937& randomGenerator,
   ...
   RHEAGenome& parent2)
{
   ...
   while(! gameState.isGameOver() && actionSpace.size() > 0 && actIdx < params.individualLength) {
      ...
      if(mutate) {
         ...
         // Apply the chosen action to the game state and generate the new set of possible actions
         // for the next step.
         applyActionToGameState(forwardModel, gameState, actions[actIdx], params);
         actionSpace = forwardModel.generateActions(gameState, params.PLAYER_ID);
      }
...
void RHEAGenome::toString() const
{
   ...
   for(const auto& action : actions) {
      std::cout << "\t\t" << action.getOwnerID() << ";"
                << "Type=" << action.getActionType().getName() << "\n";
   }
