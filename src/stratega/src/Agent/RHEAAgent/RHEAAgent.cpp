...
ActionAssignment RHEAAgent::computeAction(
...
void RHEAAgent::initializePopulation(
   const ForwardModel& forwardModel, GameState& gameState, std::mt19937&)
{
...
std::vector< RHEAGenome > RHEAAgent::shiftPopulation(
   const ForwardModel& forwardModel, GameState& gameState, std::mt19937& randomGenerator)
{
...
bool sortByFitness(const RHEAGenome& i, const RHEAGenome& j)
{
   return i.getValue() > j.getValue();
}

void RHEAAgent::rheaLoop(
   const ForwardModel& forwardModel, GameState& gameState, std::mt19937& randomGenerator)
{
...
std::vector< RHEAGenome > RHEAAgent::nextGeneration(
   const ForwardModel& forwardModel, GameState& gameState, std::mt19937& randomGenerator)
{
...
RHEAGenome RHEAAgent::getNextGenerationIndividual(
   const ForwardModel& forwardModel, GameState& gameState, std::mt19937& randomGenerator)
{
   params_.currentIterations++;
   if(params_.popSize > 1) {
      std::vector< RHEAGenome > parents = tournamentSelection();
      return RHEAGenome::crossover(
         forwardModel, gameState, params_, randomGenerator, parents[0], parents[1]);
   } else {
...
std::vector< RHEAGenome > RHEAAgent::tournamentSelection()
{
   ...
   std::sample(
      pop_.begin(),
      pop_.end(),
      std::back_inserter(tournament),
      params_.tournamentSize,
