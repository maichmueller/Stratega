...
{
   switch(budgetType) {
      case Budget::TIME: return timer.percCompletedTime() > percTimeBudget;
      case Budget::ITERATIONS: return currentIterations >= maxIterations;
      case Budget::FMCALLS: return currentFMCalls >= maxFMCalls;
   }
   return false;
}

std::unique_ptr< ForwardModel > AgentParameters::preprocessForwardModel(
   const ForwardModel& forwardModel)
{
   ...
   if(forwardModel.getGameType() == SGA::GameType::TBS)
      if(isPortfolio)
         return std::make_unique< PortfolioTBSForwardModel >(
            dynamic_cast< const TBSForwardModel& >(forwardModel), portfolio);
      else
         return std::make_unique< TBSForwardModel >(
            dynamic_cast< const TBSForwardModel& >(forwardModel));

   else if(forwardModel.getGameType() == SGA::GameType::RTS)
      if(isPortfolio)
         return std::make_unique< PortfolioRTSForwardModel >(
            dynamic_cast< const RTSForwardModel& >(forwardModel), portfolio);
      else
         return std::make_unique< RTSForwardModel >(
            dynamic_cast< const RTSForwardModel& >(forwardModel));

   else
      throw std::runtime_error("Unrecognized forward model type in Agent Parameters.");
}
...
void AgentParameters::printDetails() const
{
   ...
   std::cout << "\tMax FM Calls (" << (budgetType == Budget::FMCALLS ? "active" : "inactive")
             << "): " << maxFMCalls << std::endl;
   std::cout << "\tMax Iterations (" << (budgetType == Budget::ITERATIONS ? "active" : "inactive")
             << "): " << maxIterations << std::endl;
   std::cout << "\tStop at Perc Time (" << (budgetType == Budget::TIME ? "active" : "inactive")
             << "): " << percTimeBudget << std::endl;
