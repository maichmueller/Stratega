...
namespace SGA {
std::vector< Action > CombatAgent::filterUnitActions(
   std::vector< Action >& actions, Entity& unit) const
{
   std::vector< Action > filteredActions;
   for(const auto& a : actions) {
      if(a.getActionTypeID() != -1 && a.isEntityAction()
         && a.getTargets()[0].getEntityID() == unit.getID()) {
...
std::vector< Action > CombatAgent::filterActionTypes(
   std::vector< Action >& actions, std::string type) const
{
   std::vector< Action > filteredActions;
...
std::vector< Entity > CombatAgent::filterUnitsByReach(
   const std::vector< Entity >& targetUnits, const Vector2f& pos, GameState& /*currentState*/) const
{
...
std::vector< Action > CombatAgent::filterMovesInRange(
...
int CombatAgent::getPotentialHealing(
...
bool CombatAgent::getMoveInRange(
   Entity& u,
   const Vector2f& pos,
   int range,
   ...
   GameState& gameState) const
{
   ...
   if(inRangeMoves.empty()) {
      ...
      for(const auto& move : inRangeMoves) {
         auto score = -getPotentialDamage(
            move.getTargets()[1].getPosition(gameState), opponentUnits, gameState);
         if(score > bestScore) {
            bestScore = score;
            bucket = move;
            foundAction = true;
...
double CombatAgent::getPotentialDamage(
   const Vector2f& pos, const std::vector< Entity >& potentialAttackers, GameState& gameState) const
{
...
double CombatAgent::getAttackScore(
   std::vector< Action >& actions,
   const Entity& target,
   const Action& attack,
   const std::vector< Entity >& opponentUnits,
...
double CombatAgent::getHealScore(
   std::vector< Action >& /*actions*/,
   const Entity& target,
   const Action& heal,
   const std::vector< Entity >& opponentUnits,
   GameState& gameState) const
{
   ...
   double targetHealth = target.getParameter("Health");
   double maxHealth = (gameState.getGameInfo()->getEntityTypes())
                         .at(target.getID())
                         .getParamMax("Health");
   ...
   if(potentialDamage < 0.0001) {
      ...
      // return unitScores.at(target.typeID) * 0.5 * (resultingHealth - getHealth(&target,
      // gameState)) / static_cast<double>(healAmount);
   } else if(potentialDamage > resultingHealth) {
      // The unit will die if the opponent wants it
      // return unitScores.at(target.typeID) * 0.2 * (resultingHealth - getHealth(&target,
      // gameSgetMaxHealthtate)) / static_cast<double>(healAmount);
      ...
      int turnsUntilDeath = std::min< int >(
         4, static_cast< int >(std::ceil(targetHealth / (potentialDamage - healAmount))));
...
ActionAssignment CombatAgent::playTurn(GameState& currentState, const ForwardModel& fm)
{
   ...
   // Compute the best target that we should attack, based on how much support it has and how easy
   // we can attack it
   Entity* bestAttackTarget = nullptr;
   double highestScore = std::numeric_limits< double >::lowest();
   for(auto opp : opponentUnits) {
      // How much support has the unit? Computed by estimating how long it reaches for support to
      // arrive and how strong it is.
      ...
      // How much attack power do we have? Computed by estimating how long it takes to attack and
      // how strong our units are.
      ...
      for(auto attacker : myUnits) {
         ...
         int movesToAttack =
            (int) (std::max(0, dist - static_cast< int >(movementRange)) / movementRange);
   ...
   Action nextAction = filterActionTypes(actions, "EndTurn")
                          .at(0);  // Only one EndTurn action available
   bool foundAction = false;
   for(auto unit : myUnits) {
      ...
      for(const auto& heal : filterActionTypes(subActions, "Heal")) {
         ...
         double maxHealth = (currentState.getGameInfo()->getEntityTypes())
                               .at(targetUnit.getID())
                               .getParamMax("Health");
      ...
      double attackRange = unit.getParameter("AttackRange");
      if(getMoveInRange(
            unit, moveTarget, (int) attackRange, opponentUnits, moves, nextAction, currentState)) {
         break;
      }
   }
