...
namespace SGA {
TBSGameRunner::TBSGameRunner(const GameConfig& config) : GameRunner(config) {}
...
void TBSGameRunner::playInternal(std::vector< Agent* >& agents, int /*humanIndex*/)
{
   ...
   while(! currentState->isGameOver()) {
      ...
      {
         try {
            agentThread.startComputing(
               *currentAgent, *currentState, *forwardModel, *config, budgetTimeMs);
            // Render
            ...
            if(shouldCheckComputationTime)
               if(! checkComputationTime(results.computationTime))
                  nextAction = ActionAssignment::fromSingleAction(
                     Action::createEndAction(currentAgent->getPlayerID()));
...
void TBSGameRunner::runInternal(std::vector< Agent* >& agents, GameObserver& observer)
{
   while(! currentState->isGameOver()) {
      ...
      try {
         ...
         if(shouldCheckComputationTime)
            if(! checkComputationTime(results.computationTime))
               actionAssignment = ActionAssignment::fromSingleAction(
                  Action::createEndAction(currentAgent->getPlayerID()));
      } catch(const std::exception& ex) {
         std::cout << "Agent error: " << ex.what() << std::endl;
         return;
...
bool TBSGameRunner::checkComputationTime(std::chrono::milliseconds computationTime)
{
   ...
   if(playerWarnings[currentState->getCurrentTBSPlayer()] >= maxNumberWarnings) {
      ...
      std::cout << "WARNING: Player " << std::to_string(currentPlayerID)
                << " disqualified for exceeding warnings (" << maxNumberWarnings << ")"
                << std::endl;
      return false;
   } else if(
      computationTime.count() > budgetTimeWarningLimit
      && computationTime.count() < disqualificationBudgetTimeMs) {
      // add one warning
      playerWarnings[currentPlayerID]++;
      std::cout << "WARNING: Player " << std::to_string(currentPlayerID)
                << " has exceeded the computation time (" << computationTime.count() << ">"
      ...
      currentState->getPlayer(currentPlayerID)->setCanPlay(false);
      std::cout << "WARNING: Player " << std::to_string(currentPlayerID)
                << " disqualified for exceeding the computation time (" << computationTime.count()
