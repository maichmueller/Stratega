...
{
   if(initializationTime.count() > initBudgetTimetMs
      && initializationTime.count() < initDisqualificationBudgetTimeMs) {
      playerWarnings[playerID]++;
      std::cout << "WARNING: Player " << std::to_string(playerID)
                << " has exceeded the initialization time" << std::endl;
      ...
      std::cout << "WARNING: Player " << std::to_string(playerID)
                << " disqualified for exceeding the initialization time" << std::endl;
   }
}
   ...
   try {
      for(auto& agent : agents) {
         ...
         if(agent != nullptr) {
            ...
            auto initTime = std::chrono::duration_cast< std::chrono::milliseconds >(end - begin);
