...
namespace SGA {
RTSGameRunner::RTSGameRunner(const GameConfig& config) : GameRunner(config) {}
...
void RTSGameRunner::playInternal(std::vector< Agent* >& agents, int /*humanIndex*/)
{
   ...
   while(! currentState->isGameOver() && ! renderer->isGameEndRequested()) {
      ...
      for(size_t i = 0; i < agents.size(); i++) {
         if(agents[i] != nullptr) {
            ...
            if(! threads[i].isComputing() && threads[i].isJoined())
               threads[i].startComputing(
                  *agents[i], *currentState, *forwardModel, *config, budgetTimeMs);
         }
      }
      ...
      while(std::chrono::high_resolution_clock::now() - startTime
            < std::chrono::milliseconds(config->budgetTimeMs)) {
         renderer->render();
      }
...
bool RTSGameRunner::checkComputationTime(
   std::chrono::milliseconds computationTime, int currentPlayerID)
{
   if(playerWarnings[currentPlayerID] >= maxNumberWarnings) {
      ...
      computationTime.count() > budgetTimeMs
