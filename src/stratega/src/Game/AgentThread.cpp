   ...
   long timeBudgetMs)
{
   ...
   try {
      ...
      results.actions = agent.computeAction(
         std::move(stateCopy), forwardModel, Timer(timeBudgetMs));
...
void AgentThread::startComputing(
   Agent& newAgent,
   const GameState& newState,
   const ForwardModel& newForwardModel,
...
{
   resultCache = runAgent(
      *this->agent, *this->state, *this->forwardModel, *this->gameConfig, timeBudgetMs);
   computing = false;
}
