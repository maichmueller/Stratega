...
namespace SGA {
ResourceLowerEqual::ResourceLowerEqual(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : resourceReference(parameters.at(0)), lowerBoundParameter(parameters.at(1)), Condition(exp)
{
}

bool ResourceLowerEqual::isFullfiled(
...
ResourceGreaterEqual::ResourceGreaterEqual(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : resourceReference(parameters.at(0)), lowerBoundParameter(parameters.at(1)), Condition(exp)
{
}

bool ResourceGreaterEqual::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
   auto targetResource = resourceReference.getParameterValue(state, targets);
   double lowerBound = lowerBoundParameter.getConstant(state, targets);
...
HasElapsedTime::HasElapsedTime(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : lowerBoundParameter(parameters.at(0)), Condition(exp)
{
}

bool HasElapsedTime::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
   double lowerBound = lowerBoundParameter.getConstant(state, targets);
...
DifferentPlayer::DifferentPlayer(
   const std::string exp, const std::vector< FunctionParameter >& /*parameters*/)
    : Condition(exp)
{
}

bool DifferentPlayer::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
   auto& sourceEntity = targets[0].getEntityConst(state);
   auto& targetEntity = targets[1].getEntityConst(state);
...
IsWalkable::IsWalkable(const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetPosition(parameters[0]), Condition(exp)
{
}

bool IsWalkable::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
...
IsTile::IsTile(const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetPosition(parameters[0]), targetTile(parameters[1]), Condition(exp)
{
}
...
IsPlayerEntity::IsPlayerEntity(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetParam(parameters[0]), Condition(exp)
{
}

bool IsPlayerEntity::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
...
IsResearched::IsResearched(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : playerParam(parameters[0]), technologyTypeParam(parameters[1]), Condition(exp)
{
}

bool IsResearched::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
...
HasNoEntity::HasNoEntity(const std::string exp, const std::vector< FunctionParameter >& parameters)
    : playerParam(parameters[0]), entityTypeParam(parameters[1]), Condition(exp)
{
}

bool HasNoEntity::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
   const auto& targetPlayer = playerParam.getPlayer(state, targets);
...
CanSpawnCondition::CanSpawnCondition(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : sourceEntityParam(parameters[0]), targetEntityTypeParam(parameters[1]), Condition(exp)
{
}

bool CanSpawnCondition::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
...
CanAfford::CanAfford(const std::string exp, const std::vector< FunctionParameter >& parameters)
    : sourceParam(parameters[0]), costParam(parameters[1]), Condition(exp)
{
}

bool CanAfford::isFullfiled(
   const GameState& state, const std::vector< ActionTarget >& targets) const
{
