...
void RTSForwardModel::moveEntities(GameState& state) const
{
   for(auto& entity : state.getEntities()) {
      ...
      auto targetPos = Vector2f(
         entity.getPath().m_straightPath[cPathIdx * 3],
         entity.getPath().m_straightPath[cPathIdx * 3 + 2]);
      ...
      if(movementDistance <= movementSpeed) {
         ...
         entity.setPosition(
            entity.getPosition() + (movementDir / movementDir.magnitude()) * movementSpeed);
      }
   }
}
   ...
   for(auto& unit : state.getEntities()) {
      ...
      for(auto& otherUnit : state.getEntities()) {
         ...
         if(dir.magnitude() <= unit.getCollisionRadius() + otherUnit.getCollisionRadius()) {
            auto penetrationDepth = unit.getCollisionRadius() + otherUnit.getCollisionRadius()
                                    - dir.magnitude();
...
void RTSForwardModel::resolveEnvironmentCollisions(GameState& state) const
{
   ...
   // Collision
   for(auto& unit : state.getEntities()) {
      int startCheckPositionX = static_cast< int >(
         std::floor(unit.x() - unit.getCollisionRadius() - RECT_SIZE));
      int endCheckPositionX = static_cast< int >(
         std::ceil(unit.x() + unit.getCollisionRadius() + RECT_SIZE));
      int startCheckPositionY = static_cast< int >(
         std::floor(unit.y() - unit.getCollisionRadius() - RECT_SIZE));
      int endCheckPositionY = static_cast< int >(
         std::ceil(unit.y() + unit.getCollisionRadius() + RECT_SIZE));
...
bool RTSForwardModel::buildNavMesh(GameState& state, NavigationConfig config) const
{
   ...
   nav->m_cfg.mergeRegionArea = (int) rcSqr(
      nav->config.m_regionMergeSize);  // Note: area = size*size
   nav->m_cfg.maxVertsPerPoly = (int) nav->config.m_vertsPerPoly;
   nav->m_cfg.detailSampleDist = nav->config.m_detailSampleDist < 0.9f
                                    ? 0
                                    : nav->config.m_cellSize * nav->config.m_detailSampleDist;
   ...
   rcCalcGridSize(
      nav->m_cfg.bmin, nav->m_cfg.bmax, nav->m_cfg.cs, &nav->m_cfg.width, &nav->m_cfg.height);
   ...
   if(nav->config.m_filterLedgeSpans)
      rcFilterLedgeSpans(
         &nav->m_ctx, nav->m_cfg.walkableHeight, nav->m_cfg.walkableClimb, *nav->m_solid);
   ...
   if(! rcBuildCompactHeightfield(
         &nav->m_ctx,
         nav->m_cfg.walkableHeight,
         nav->m_cfg.walkableClimb,
         *nav->m_solid,
         *nav->m_chf)) {
   ...
   // Partition the heightfield so that we can use simple algorithm later to triangulate the
   // walkable areas. There are 3 martitioning methods, each with some pros and cons: 1) Watershed
   // partitioning
   ...
   //      - holes may appear when a small obstacles is close to large open area (triangulation can
   //      handle this)
   //      - overlaps may occur if you have narrow spiral corridors (i.e stairs), this make
   //      triangulation to fail
   //   * generally the best choice if you precompute the nacmesh, use this if you have large open
   //   areas
   ...
   if(nav->config.m_partitionType == SAMPLE_PARTITION_WATERSHED) {
      ...
      if(! rcBuildRegions(
            &nav->m_ctx, *nav->m_chf, 0, nav->m_cfg.minRegionArea, nav->m_cfg.mergeRegionArea)) {
   ...
   if(! rcBuildContours(
         &nav->m_ctx,
         *nav->m_chf,
         nav->m_cfg.maxSimplificationError,
         nav->m_cfg.maxEdgeLen,
         *nav->m_cset)) {
   ...
   if(! rcBuildPolyMeshDetail(
         ...
         *nav->m_pmesh,
         *nav->m_chf,
         nav->m_cfg.detailSampleDist,
         nav->m_cfg.detailSampleMaxError,
         *nav->m_dmesh)) {
...
Path RTSForwardModel::findPath(const GameState& state, Vector2f startPos, Vector2f endPos) const
{
   ...
   nav->m_navQuery->findNearestPoly(
      startPosV3, nav->m_polyPickExt, &nav->m_filter, &startRef, startPosV3);
   nav->m_navQuery->findNearestPoly(
      endPosV3, nav->m_polyPickExt, &nav->m_filter, &endRef, endPosV3);

   Path path;

   if(startRef && endRef) {
      nav->m_navQuery->findPath(
         startRef, endRef, startPosV3, endPosV3, &nav->m_filter, m_polys, &m_npolys, MAX_POLYS);
      ...
      if(m_npolys) {
         ...
         nav->m_navQuery->findStraightPath(
            startPosV3,
            endPosV3,
            m_polys,
            m_npolys,
            ...
            path.m_straightPathPolys,
            &path.m_nstraightPath,
            MAX_POLYS,
            path.m_straightPathOptions);
      }
   } else {
      path.m_nstraightPath = 0;
   }
