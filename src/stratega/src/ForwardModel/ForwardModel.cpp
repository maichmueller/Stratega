...
namespace SGA {
ForwardModel::ForwardModel() : actionSpace(generateDefaultActionSpace()) {}
...
void ForwardModel::generateActions(
...
void ForwardModel::executeContinuousAction(GameState& state, const Action& action) const
{
   ...
   newAction.getTargets().emplace_back(
      ActionTarget::createContinuousActionActionTarget(newAction.getContinuousActionID()));
...
void ForwardModel::executeOnTriggerEffects(GameState& state) const
{
   ...
   for(const auto& onTickEffect : onTickEffects) {
      for(const auto& entity : state.getEntities()) {
         if(onTickEffect.validTargets.find(entity.getEntityTypeID())
            == onTickEffect.validTargets.end())
            continue;
         std::vector< ActionTarget > targets;
   ...
   for(auto et = entities.begin(); et != entities.end(); et++) {
      ...
      while(it != continuousActions.end()) {
         ...
         if(isComplete) {
            ...
            state.entities[j].continuousAction[i].targets))
                    {
                            canExecute = false;
                            break;
                    }
   ...
   for(size_t j = 0; j < players.size(); j++) {
      ...
      for(size_t i = 0; i < continuousActions.size(); i++) {
         ...
         if(isComplete) {
            ...
            {
                    if (!condition->isFulfilled(state,
            state.players[j].continuousAction[i].targets))
                    {
                            canExecute = false;
                            break;
                    }
