         ...
         if(actionType.isContinuous()) {
            ...
            for(const auto& action : sourceEntity.getContinuousActions()) {
               if(action.getActionTypeID() == actionType.getID()) {
                  ...
                  bucket.emplace_back(Action::createAbortAction(
                     playerID, sourceEntity.getID(), action.getContinuousActionID()));
               }
            }
         }
      ...
      if(actionType.isContinuous()) {
         ...
         for(auto& action : player.getContinuousActions()) {
            if(action.getActionTypeID() == actionType.getID()) {
               ...
               // Give the posibility to abort it
               bucket.emplace_back(
                  Action::createAbortAction(player.getID(), action.getContinuousActionID()));
            }
         }
      }
...
auto productActionTargets(const std::vector< std::vector< ActionTarget > >& lists)
{
   ...
   if(std::find_if(std::begin(lists), std::end(lists), [](auto e) -> bool { return e.empty(); })
      != std::end(lists)) {
      return result;
   }
...
void ActionSpace::generateActions(
   const GameState& state,
   const Entity& sourceEntity,
   const ActionType& actionType,
   const std::vector< std::vector< ActionTarget > >& targets,
   std::vector< Action >& actionBucket) const
{
   for(auto& targetsProduct : productActionTargets(targets)) {
      Action action(&actionType);
      action.setOwnerID(sourceEntity.getOwnerID());
      action.getTargets().emplace_back(
         ActionTarget::createEntityActionTarget(sourceEntity.getID()));
   ...
   std::vector< Action >& actionBucket) const
{
   for(auto& targetsProduct : productActionTargets(targets)) {
      ...
      action.getTargets().emplace_back(
         ActionTarget::createPlayerActionTarget(sourcePlayer.getID()));
...
std::vector< std::vector< ActionTarget > > ActionSpace::generateTargets(
   const GameState& state, const Entity& entity, const ActionType& action) const
{
   ...
   for(auto& type : action.getTargets()) {
      ...
      switch(type.first.getType()) {
         case TargetType::Position:
            newTargets = generatePositionTargets(
               state, entity.getPosition(), type.first.getSamplingMethod());
            break;
         case TargetType::Entity:
            newTargets = generateGroupTargets(
               state,
               entity.getPosition(),
               type.first.getGroupEntityTypes(),
               type.first.getSamplingMethod());
            break;
         case TargetType::EntityType:
            newTargets = generateEntityTypeTargets(state, type.first.getGroupEntityTypes());
            break;
         case TargetType::Technology:
            newTargets = generateTechnologyTargets(state, type.first.getTechnologyTypes());
            break;
         case TargetType::ContinuousAction:
            newTargets = generateContinuousActionTargets(state, entity);
            break;
...
std::vector< std::vector< ActionTarget > > ActionSpace::generateTargets(
   const GameState& state, const Player& /*entity*/, const ActionType& action) const
{
   std::vector< std::vector< ActionTarget > > targets;
   ...
   for(auto& type : action.getTargets()) {
      ...
      switch(type.first.getType()) {
         case TargetType::Position:
            newTargets = generatePositionTargets(state, type.first.getSamplingMethod());
            break;
         case TargetType::Entity:
            newTargets = generateGroupTargets(
               state, type.first.getGroupEntityTypes(), type.first.getSamplingMethod());
            break;
         case TargetType::EntityType:
            newTargets = generateEntityTypeTargets(state, type.first.getGroupEntityTypes());
            break;
...
std::vector< ActionTarget > ActionSpace::generatePositionTargets(
   const GameState& gameState,
   const Vector2f& position,
   std::shared_ptr< SamplingMethod > samplingMethod) const
{
   ...
   for(auto& element : positions) {
      targets.emplace_back(
         ActionTarget::createPositionActionTarget(Vector2f(element.x, element.y)));
   }
...
std::vector< ActionTarget > ActionSpace::generatePositionTargets(
   const GameState& gameState, std::shared_ptr< SamplingMethod > samplingMethod) const
{
   std::vector< ActionTarget > targets;
...
std::vector< ActionTarget > ActionSpace::generateEntityTypeTargets(
   const GameState& /*gameState*/, const std::unordered_set< EntityTypeID >& entityTypeIDs) const
{
   std::vector< ActionTarget > targets;
...
std::vector< ActionTarget > ActionSpace::generateGroupTargets(
   const GameState& gameState,
   const std::unordered_set< EntityTypeID >& entityTypeIDs,
   std::shared_ptr< SamplingMethod > samplingMethod) const
{
   std::vector< ActionTarget > targets;
   ...
   for(const auto& entityID : entitiesIDs) {
      if(entityTypeIDs.find(gameState.getEntityConst(entityID)->getEntityTypeID())
         != entityTypeIDs.end()) {
...
std::vector< ActionTarget > ActionSpace::generateTechnologyTargets(
   const GameState& gameState, const std::unordered_set< int >& entityTypeIDs) const
{
   std::vector< ActionTarget > targets;

   for(const auto& technoloTreeType :
       gameState.getGameInfo()->getTechnologyTreeCollection().technologyTreeTypes) {
      for(auto& technology : technoloTreeType.second.technologies) {
         if(entityTypeIDs.find(technology.second.id) != entityTypeIDs.end()) {
            targets.emplace_back(
               ActionTarget::createTechnologyEntityActionTarget(technology.second.id));
         }
      }
   }
   return targets;
...
std::vector< ActionTarget > ActionSpace::generateContinuousActionTargets(
   const GameState& /*gameState*/, const Entity& sourceEntity) const
{
   std::vector< ActionTarget > targets;

   for(const auto& action : sourceEntity.getContinuousActions()) {
      targets.emplace_back(
         ActionTarget::createContinuousActionActionTarget(action.getContinuousActionID()));
   }
   return targets;
}

Action ActionSpace::generateSelfAction(
   const Entity& sourceEntity, const ActionType& actionType) const
{
...
Action ActionSpace::generateSelfAction(
   const Player& sourcePlayer, const ActionType& actionType) const
{
   Action selfAction(&actionType);
   std::vector< ActionTarget > actionTargets;
