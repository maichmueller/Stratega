...
{
   if(m_targetType == PlayerReference) {
      return m_data.playerID;
   }

   throw std::runtime_error(
      "Target type " + std::to_string(int(m_targetType)) + " not recognised in action target.");
}
...
int ActionTarget::getPlayerID(const GameState& state) const
{
   if(m_targetType == PlayerReference) {
      return m_data.playerID;
   } else if(m_targetType == EntityReference) {
      return state.getEntityConst(m_data.entityID)->getOwnerID();
   }
...
const std::unordered_set< EntityTypeID >& ActionTarget::getSpawnableEntities(
...
{
   if(m_targetType == Type::EntityReference) {
      return *state.getEntityConst(m_data.entityID);
   } else {
      throw std::runtime_error(
         "Target type " + std::to_string(int(m_targetType)) + " not recognised in action target.");
   }
...
{
   if(m_targetType == Type::EntityReference) {
      auto* entity = state.getEntity(m_data.entityID);
...
{
   if(m_targetType == Type::PlayerReference) {
      auto* player = state.getPlayer(m_data.playerID);
      if(player == nullptr) {
         throw std::runtime_error(
            "An action-target contained an not existing player with id "
            + std::to_string(m_data.playerID));
      }
      return *player;
   } else {
      throw std::runtime_error(
...
{
   if(m_targetType == EntityTypeReference) {
      const auto& type = state.getGameInfo()->getEntityType(m_data.entityTypeID);
      return type;
   } else if(m_targetType == EntityReference) {
      const auto& type = state.getEntityConst(m_data.entityID)->getEntityType();
      return type;
   } else {
      throw std::runtime_error(
         "Target type " + std::to_string(int(m_targetType)) + " not recognised in action target.");
...
{
   if(m_targetType == TileTypeReference) {
      const auto& type = state.getGameInfo()->getTileType(m_data.entityTypeID);
      return type;
   } else {
      throw std::runtime_error(
         "Target type " + std::to_string(int(m_targetType)) + " not recognised in action target.");
...
{
   switch(m_targetType) {
      case EntityReference: return state.getEntityConst(m_data.entityID); break;
      default: return true;
   }
}

bool ActionTarget::isValidWithTargets(
   const GameState& state,
   const ActionType& actionType,
   const std::vector< ActionTarget >& actionTargets)
{
   auto isValid = true;
   int i = 0;
...
std::string ActionTarget::getValueString(const GameState& state) const
{
   switch(m_targetType) {
      ...
      case PlayerReference: return std::to_string(getPlayerID()); break;
      ...
      case TechnologyReference: {
         ...
         std::string technologyName = state.getGameInfo()
                                         ->getTechnologyTreeCollection()
                                         .getTechnology(technologyID)
                                         .name;
         return technologyName;
      } break;
      case ContinuousActionReference: return std::to_string(getContinuousActionID()); break;
      case TileTypeReference: return getTileType(state).getName(); break;
