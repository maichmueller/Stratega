...
ModifyResource::ModifyResource(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : resourceReference(parameters.at(0)), amountParameter(parameters.at(1)), Effect(exp)
{
}

void ModifyResource::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
...
void AttackProbability::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
   auto& entity = resourceReference.getEntity(state, targets);
   auto& targetResource = resourceReference.getParameterValue(state, targets);
...
Move::Move(const std::string exp, const std::vector< FunctionParameter >& parameters)
    : entityParam(parameters[0]), targetPositionParam(parameters[1]), Effect(exp)
{
}

void Move::execute(
   GameState& state, const ForwardModel& fm, const std::vector< ActionTarget >& targets) const
{
   ...
   if(const auto* tbsFM = dynamic_cast< const TBSForwardModel* >(&fm)) {
      ...
      oldTargetPos.y = entity.getPath()
                          .m_straightPath[((entity.getPath().m_nstraightPath - 1) * 3) + 2];

      // Compute a new path if the entity doesn't have one or the new target is different from the
      // old one
...
SetToMaximum::SetToMaximum(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetResource(parameters[0]), Effect(exp)
{
}

void SetToMaximum::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
...
TransferEffect::TransferEffect(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : sourceParam(parameters[0]),
      ...
      amountParam(parameters[2]),
      Effect(exp)
{
}

void TransferEffect::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
   ...
   // Compute how much the source can transfer, if the source does not have enough just take
   // everything
   amount = std::min(amount, sourceValue - sourceType.getMinValue());
   // Transfer
   sourceValue -= amount;
   // ToDo should check the maximum, but currently we have no way to set the maximum in the
   // configuration Resulting in problems for ProtectTheBase
   targetValue = targetValue + amount;
}

ChangeOwnerEffect::ChangeOwnerEffect(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetEntityParam(parameters[0]), playerParam(parameters[1]), Effect(exp)
{
}

void ChangeOwnerEffect::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
...
RemoveEntityEffect::RemoveEntityEffect(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : targetEntityParam(parameters[0]), Effect(exp)
{
}

void RemoveEntityEffect::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
   auto& targetEntity = targetEntityParam.getEntity(state, targets);
...
ResearchTechnology::ResearchTechnology(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : playerParam(parameters[0]), technologyTypeParam(parameters[1]), Effect(exp)
{
}

void ResearchTechnology::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
...
SpawnEntityRandom::SpawnEntityRandom(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : sourceEntityParam(parameters[0]), targetEntityTypeParam(parameters[1]), Effect(exp)
{
}

void SpawnEntityRandom::execute(
   GameState& state, const ForwardModel& fm, const std::vector< ActionTarget >& targets) const
{
   if(const auto* tbsFM = dynamic_cast< const TBSForwardModel* >(&fm)) {
      ...
      for(int dx = -1; dx <= 1; dx++) {
         for(int dy = -1; dy <= 1; dy++) {
            Vector2i spawnPos{
               static_cast< int >(sourceEntity.x()) + dx,
               static_cast< int >(sourceEntity.y()) + dy};
            if(! state.isInBounds(spawnPos))
               continue;
            if(! state.isWalkable(spawnPos))
               continue;

            fm.spawnEntity(
               state,
               targetEntityType,
               sourceEntity.getOwnerID(),
               Vector2f(spawnPos.x, spawnPos.y));
            return;
         }
      }
   } else {
...
SpawnEntityGrid::SpawnEntityGrid(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : spawnSource(parameters[0]),
      entityTypeParam(parameters[1]),
      targetPositionParam(parameters[2]),
...
PayCostEffect::PayCostEffect(
   const std::string exp, const std::vector< FunctionParameter >& parameters)
    : sourceParam(parameters[0]), costParam(parameters[1]), Effect(exp)
{
}

void PayCostEffect::execute(
   GameState& state, const ForwardModel&, const std::vector< ActionTarget >& targets) const
{
