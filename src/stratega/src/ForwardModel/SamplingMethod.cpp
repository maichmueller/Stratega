...
std::vector< int > SGA::SamplingMethod::getEntities(
...
bool SGA::SamplingMethod::validatePosition(
   const GameState& /*gameState*/,
   const Vector2f& /*sourcePosition*/,
   const Vector2f& /*targetPosition*/) const
{
   return true;
}

bool SGA::SamplingMethod::validatePosition(
   const GameState& gameState, const Vector2f& targetPosition) const
{
   return gameState.getTileAt({(int) targetPosition.x, (int) targetPosition.y}).getTileTypeID()
          != -1;
}

std::vector< SGA::Vector2i > SGA::Neighbours::getPositions(
   const GameState& gameState, const Vector2f& position) const
{
   std::vector< SGA::Vector2i > targets;
   ...
   if(shapeType == ShapeType::AllPositions) {
      ...
      auto endCheckPositionX = std::min< int >(
         static_cast< int >(gameState.getBoardWidth() - 1),
         static_cast< int >(position.x + shapeSize));
      auto startCheckPositionY = std::max< int >(0, static_cast< int >(position.y - shapeSize));
      auto endCheckPositionY = std::min< int >(
         static_cast< int >(gameState.getBoardHeight() - 1),
         static_cast< int >(position.y + shapeSize));
...
std::vector< int > SGA::Neighbours::getEntities(
   const GameState& gameState,
   const Vector2f& position,
   const std::unordered_set< int >& /*entityTypeIDs*/) const
{
   std::vector< int > entitiesIDs;
   // Call base method
   for(auto& entity : gameState.getEntities()) {
      if(shapeType == ShapeType::AllPositions
         || entity.getPosition().distance(position) <= shapeSize)
         entitiesIDs.emplace_back(entity.getID());
   }
   return entitiesIDs;
}

bool SGA::Neighbours::validatePosition(
   const GameState& /*gameState*/,
   const Vector2f& sourcePosition,
   const Vector2f& targetPosition) const
{
   switch(shapeType) {
      case ShapeType::Square: {
         return (
            sourcePosition.x >= targetPosition.x - shapeSize
            && sourcePosition.x <= targetPosition.x + shapeSize
            && sourcePosition.y >= targetPosition.y - shapeSize
            && sourcePosition.y <= targetPosition.y + shapeSize);
      }
...
std::vector< SGA::Vector2i > SGA::Dijkstra::getPositions(
   const GameState& gameState, const Vector2f& position) const
{
...
std::vector< int > SGA::Dijkstra::getEntities(
   const GameState& gameState,
   const Vector2f& position,
   const std::unordered_set< int >& /*entityTypeIDs*/) const
{
...
bool SGA::Dijkstra::validatePosition(
   const GameState& gameState, const Vector2f& sourcePosition, const Vector2f& targetPosition) const
{
