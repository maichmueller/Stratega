...
class TextureAtlas {
  ...
  public:
   ...
   void init(const std::vector< std::string >& spritePaths)
   {
      ...
      // Since we don't want to split up a texture, turn the square into a slightly inaccurate
      // rectangle
      ...
      for(const auto& path : spritePaths) {
         ...
         if(image.getSize() != spriteSize) {
            throw std::runtime_error(
               ...
               + ". Textures added to the atlas need to have the same size: <"
   ...
   sf::Rect< float > getSpriteRect(const std::string& spritePath) const
   {
      assert(initialized);
      return rectLookup.at(spritePath);
   }
