...
namespace SGA {
enum class GameType { TBS, RTS };

/// <summary>
/// Contains the game data without any logic, offering access to the current board, a list of player
/// and their units. If the agent want access to the definition of entity types, actions or game
/// config yaml  it should access to <see cref="SGA::GameInfo"/>
/// </summary>
struct GameState final {
  public:
   ...
   auto getBoardWidth() const { return board.getWidth(); }

   /// <summary>
   /// Returns the height of the board.
   /// </summary>
   /// <returns>The height of the board.</returns>
   auto getBoardHeight() const { return board.getHeight(); }

   /// <summary>
   /// Returns the tile at the position indicated in the parameter. Can throw an exception if out of
   /// bounds.
   /// </summary>
   ...
   /// <summary>
   /// Returns the tile at the position (x,y) indicated in the parameter. Can throw an exception if
   /// out of bounds.
   /// </summary>
   ...
   /// <param name="maxDistance">If provided, considers units at a distance less or equal this value
   /// to the position provided.</param> <returns>A pointer to the entity in this
   /// location.</returns>
   Entity* getEntity(Vector2f pos, float maxDistance = 0.0);
   const Entity* getEntityAt(const Vector2f& pos) const;

   /// <summary>
   /// Returns an entity by its ID. It'll return nullptr if no entity exists associated to the given
   /// ID.
   /// </summary>
   ...
   /// <summary>
   /// Adds a new entity of a given type to the game, in a given position, belonging to a specific
   /// player.
   /// </summary>
   ...
   /// <param name="entityCategory">Entites retrieved will belong to this indicated category. If not
   /// suplied, this method returns all entities.</param> <returns>The list of entities of the given
   /// player. Returns an empty list if player ID doesn't exist or it has not entities.</returns>
   std::vector< Entity > getPlayerEntities(
      int playerID, EntityCategory entityCategory = EntityCategory::Null) const;
   ...
   /// <param name="entityCategory">Entites retrieved will belong to this indicated category. If not
   /// suplied, this method returns all entities.</param> <returns>The list of entities not own by
   /// the given player.</returns>
   std::vector< Entity > getNonPlayerEntities(
      int playerID, EntityCategory entityCategory = EntityCategory::Null) const;
   ...
   {
      return const_cast< Player* >(const_cast< const GameState* >(this)->getPlayer(playerID));
   }
   ...
   auto getNumPlayers() const { return players.size(); };

   /// <summary>
   /// Adds a player to the game state.
   /// </summary>
   /// <param name="p">The player to add.</param>
   /// <returns>Returns the ID of the player added to the game, equal to the number of players
   /// -1.</returns>
   int addPlayer(Player& p);

   /// <summary>
   /// Gets the value of a player parameter.
   /// Be sure to check first is the parameter you are asking for exist using <here
   /// cref="SGA::GameState::hasPlayerParameter()"/>
   /// </summary>
   ...
   /// <returns>Returns the value of the parameter indicated for the player whose ID is
   /// given.</returns>
   ...
   /// For the moment, all players have the same parameter names (hence playerID is not
   /// used).</param> <returns>A vector with all the parameter names of the requested
   /// player.</returns>
  ...
  private:
   ...
   /// <summary>
   /// Board: a 2 dimensional grid of tiles. This does not contain information about entities on
   /// those tiles.
   /// </summary>
   Grid2D< Tile > board;

   /// <summary>
   /// Map that indicates if a technology is researched by a player. Key is the player ID and maps
   /// to a vector of technology IDs.
   /// </summary>
   ...
   /// 	-1 if more than one can play. Use gameState.whoCanPlay() to retrieve this in vector form
   /// with N-1 player IDs.
   /// </summary>
   int currentPlayer;
