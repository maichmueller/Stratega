...
/// <summary>
/// An abstract class that provides a simplified interface to play games with a given <see
/// cref="SGA::GameConfig"/>. Use "createGameRunner" to create an instance of this class.
/// </summary>
class GameRunner {
  private:
  ...
  public:
   ...
   /// When the game has ended, you are responsible for calling <see
   /// cref="SGA::GameRunner::reset()"> to reset the environments state.
   /// </summary>
   /// <param name="actions">The action used to advance the game. Depending on the game type, the
   /// action is validated differently.</param>
   ...
   /// </summary>
   /// <param name="begin">The beginning of the range containing the agents to obtain actions
   /// from</param> <param name="end">The end of the range containing the agents to obtain actions
   /// from</param> <param name="observer">An optional observer who is notified about important
   /// events in the game.</param>
   ...
   /// </summary>
   /// <param name="begin">The beginning of a range containing agents to obtain actions from.
   /// Exactly one agent can be left empty, which is then controlled by the human player.</param>
   /// <param name="end">The end of the range containing the agents to obtain actions from.</param>
   /// <param name="resolution">The resolution of the window that should be used to display the
   /// game.</param>
   template < typename Iterator >
            ...
            if(agents[i] == nullptr) {
               if(humanIndex != GameRenderer::NO_PLAYER_ID) {
                  throw std::runtime_error(
                     "Only one player can be controlled by a human. Index "
                     + std::to_string(humanIndex) + " is already empty.");
               }
               humanIndex = i;
            } else {
               agents[i]->setPlayerID(i);
...
/// </summary>
/// <param name="config">The configuration file that should be used by the GameRunner. The given
/// configuration has to be kept in memory while this object exists.</param> <returns>A pointer to
/// the initialized GameRunner.</returns>
