...
/// Each <see cref="SGA::RTSForwardModel::advanceGameState()"/> call move all the entities and
/// resolve the collisions before executing all the received actions. After executing the actions it
/// updates the entities that should be removed before checking if the game is over. The <see
/// cref="SGA::RTSForwardModel::advanceGameState()"/> accepts a list of actions or one single
/// action.
/// </summary>
class RTSForwardModel: public ForwardModel {
  ...
  public:
   RTSForwardModel() : deltaTime(1. / 60.) {}

   /// <summary>
   /// Moves all the entities and resolves collisions before and after executing an action in a
   /// given <see cref="SGA::GameState"/> After executing the actions it updates the entities that
   /// should be removed before checking if the game is over.
   /// </summary>
   void advanceGameState(GameState& state, const Action& action) const override;

   /// <summary>
   /// Moves all the entities and resolves collisions before and after executing a list of actions
   /// in a given <see cref="SGA::GameState"/> After executing the actions it updates the entities
   /// that should be removed before checking if the game is over.
   /// </summary>
