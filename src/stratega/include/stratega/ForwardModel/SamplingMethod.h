...
struct SamplingMethod {
   ...
   // Based in position
   virtual std::vector< Vector2i > getPositions(
      const GameState& gameState, const Vector2f& position) const = 0;
   virtual std::vector< int > getEntities(
      const GameState& gameState,
      const Vector2f& position,
      const std::unordered_set< int >& entityTypeIDs) const = 0;
...
struct Neighbours: public SamplingMethod {
   enum class ShapeType { Circle, Square, AllPositions };
   ...
   std::vector< Vector2i > getPositions(
      const GameState& gameState, const Vector2f& position) const override;
   ...
   bool validatePosition(
      const GameState& gameState,
      const Vector2f& sourcePosition,
      const Vector2f& targetPosition) const override;
};
...
struct Dijkstra: public SamplingMethod {
   ...
   struct Node {
      ...
      inline bool operator==(const Node& rhs) const { return posX == rhs.posX && posY == rhs.posY; }
