...
class FunctionParameter {
  public:
   enum class Type {
      ...
      ParameterReference,  // References the parameter of an action-target. Source.Gold references
                           // to the gold of the source of an action
      EntityPlayerReference,  // References the player indirectly. Like Source.Player, only works if
                              // Source is a entity
      EntityPlayerParameterReference,  // References the parameter of an player indirectly. Like
                                       // Source.Player.Gold, only works if Source is a entity
      ArgumentReference,  // References an action-argument, like Source or Target. Can be used for
                          // example if you want to pass a position-target
      EntityTypeReference,  // References EntityTypes defined in the Game. Like Warrior, GoldMine,
                            // etc.
  ...
  private:
   struct ParameterReference {
      ...
      ParameterReference(ParameterID parameterID_, int argumentIndex_)
          : parameterID(parameterID_), argumentIndex(argumentIndex_)
      {
      }
   };
   ...
   struct ContinuousActionReference {
      ...
      ContinuousActionReference(int sourceEntity_, int actionID_)
          : sourceEntity(sourceEntity_), actionID(actionID_)
      {
      }
   };
   ...
   FunctionParameter(const Type& type, const Data& data) : m_parameterType(type), m_data(data){};
  ...
  public:
   ...
   const std::unordered_set< EntityTypeID >& getSpawnableEntities(
      const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   ...
   const TechnologyTreeNode& getTechnology(
      const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   const std::unordered_map< ParameterID, double >& getCost(
      const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   const std::unordered_map< ParameterID, Parameter >& getParameterLookUp(
      const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   double getTime(const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   std::vector< double >& getParameterList(
      GameState& state, const std::vector< ActionTarget >& actionTargets) const;
   const std::vector< double >& getParameterList(
      const GameState& state, const std::vector< ActionTarget >& actionTargets) const;
