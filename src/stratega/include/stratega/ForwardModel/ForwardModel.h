...
namespace SGA {
enum class WinConditionType { LastManStanding, UnitAlive };
...
/// <summary>
/// Is the driving component of Stratega, with a provided gamestate it can generate a new set of
/// available actions thanks to the <see cref="SGA::ActionSpace"/>. The returned actions store the
/// action type and a list of sources and targets the action will be applied to. Each <see
/// cref="SGA::RTSForwardModel"/> and <see cref="SGA::TBSForwardModel"/> by default contains the
/// definition of how to advance the game and other utility methods used in each specific game type.
/// When a player provides the forward model an action and a gamestate, it first checks the type of
/// action and after it updates the action info before executing the action. The forward models can
/// be extended by implementing forward model abstractions, can be useful for reducing the games
...
class ForwardModel {
  public:
   ...
   /// Advances the game state passed by parameter with a given action
   /// </summary>
   /// <param name="state">Game state to advance. The actual object is modified by this
   /// call.</param> <param name="action">ActionAssignment to execute in the game state.</param>
   ...
   /// <returns>The list of available actions for player with ID 'playerID' in game state
   /// 'state'.</returns>
   [[nodiscard]] std::vector< Action > generateActions(const GameState& state, int playerID) const;

   /// <summary>
   /// Generates actions in the given gamestate by the received player and fills the action vector
   /// passed by parameter.
   /// </summary>
   ...
   virtual void generateActions(
      const GameState& state, int playerID, std::vector< Action >& actionBucket) const;
   ...
   /// <summary>
   /// Spawns an entity in the game with the default spawn entity method. A list of On Spawn effects
   /// are executed just after spawning the entity
   /// </summary>
   ...
   void spawnEntity(
      GameState& state, const EntityType& entityType, int playerID, const Vector2f& position) const;
   ...
   /// </summary>
   /// <param name="conditions">List of conditions that, once fullfiled, cause a player to
   /// WIN.</param>
   ...
   /// conditions that must be fulfilled for the game to be over for a player. The game will be WON
   /// by a player if all conditions in a sub-list are fullfiled, for at least one of the sub-lists
   /// returned.
   /// </summary>
   /// <returns>A list of sub-lists with all WIN conditions</returns>
   const std::vector< std::vector< std::shared_ptr< Condition > > >& getWinConditions() const
   {
   ...
   /// conditions that must be fulfilled for the game to be over for a player. The game will be LOST
   /// by a player if all conditions in a sub-list are fullfiled, for at least one of the sub-lists
   /// returned.
   /// </summary>
   /// <returns>A list of sub-lists with all LOSE conditions</returns>
   const std::vector< std::vector< std::shared_ptr< Condition > > >& getLoseConditions() const
   {
   ...
   /// <summary>
   /// Adds an OnEntitySpawnEffect to the forward mode, which will be executed every time an entity
   /// is spawned.
   /// </summary>
   /// <param name="ote">Effect to add.</param>
   ...
   /// </summary>
   const std::vector< OnEntitySpawnEffect >& getOnEntitySpawnEffects() const
   {
      return onEntitySpawnEffects;
   }
  ...
  protected:
   ...
   /// <summary>
   /// Executes the action in the given game state. This is used by subclasses of this forward model
   /// to actually execute an action in the game.
   /// </summary>
   /// <param name="state">State where the action is executed. This game state will be
   /// altered.</param> <param name="action">Action to execute.</param>
