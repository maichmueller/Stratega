...
/// <summary>
/// Contains the definition of an action. Which type is the source <see
/// cref="SGA::ActionSourceType"/> , a set effects, preconditions and a list of action targets
/// linked to their target conditions. If the action is continuous it will have a set of effects as
/// events: OnStart, OnTick, OnComplete, OnAbort and the list of conditions that triggers the
/// completed action.
/// </summary>
struct ActionType {
  private:
   ...
   /// <summary>
   /// List of preconditions for this action to be run. These are tier 1 conditions: if they can't
   /// be fulfilled the action is not possible. If the conditions are true, the action may be
   /// possible based on situational circumstances. (Example: having a resource to create an entity
   /// but no space in the board to spawn it).
   /// </summary>
   std::vector< std::shared_ptr< Condition > > preconditions;

   /// <summary>
   /// List of target types of this action with their conditions. Each target is a pair <target,
   /// list<conditions>>. These are tier 2 conditions (i.e. situational), to be checked during
   /// action generation.
   /// </summary>
   std::vector< std::pair< TargetType, std::vector< std::shared_ptr< Condition > > > >
      actionTargets;

   /// <summary>
   /// Indicates if action is continuous: if the execution of this action extends beyond the tick it
   /// was started.
   /// </summary>
   bool continuous;
   ...
   /// <summary>
   /// For continuous actions, list of effects that are applied on every tick/turn. This effect is
   /// *not* applied on the tick this action starts, but it *will* also be applied in the tick it
   /// ends.
   /// </summary>
   ...
   /// <summary>
   /// For continuous actions, list of effects that are applied when this action is aborted. Aborted
   /// actions do not execute the OnTick effect in the game tick they have been aborted.
   /// </summary>
  ...
  public:
   ...
   const std::vector< std::shared_ptr< Condition > >& getTargetConditions(
      const TargetType& searchingTarget) const
   {
   ...
   const std::vector< std::shared_ptr< Condition > >& getPreconditions() const
   {
      return preconditions;
   }
   std::vector< std::shared_ptr< Condition > >& getPreconditions() { return preconditions; }

   const std::vector< std::pair< TargetType, std::vector< std::shared_ptr< Condition > > > >&
   getTargets() const
   {
      return actionTargets;
   }
   std::vector< std::pair< TargetType, std::vector< std::shared_ptr< Condition > > > >& getTargets()
   {
      return actionTargets;
   }

   const std::vector< std::shared_ptr< Condition > >& getTriggerComplete() const
   {
