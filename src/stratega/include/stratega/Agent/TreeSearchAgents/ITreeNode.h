...
class ITreeNode {
  public:
   ...
   virtual std::vector< Action > getActionSpace(
      const ForwardModel& /*forwardModel*/, int /*playerID*/) const
   {
      return actionSpace;
   }
   ...
   void printTree() const { printTree("", this, true, "root"); };
  ...
  protected:
   ...
   ///  - If the owner can't play at this state, the player that can play if there's only one other
   ///  to play.
   /// </summary>
   std::vector< Action > actionSpace;
   int playerID;  // ID of the player whose action space has been calculated in this node (and it's
                  // to move in the game).
