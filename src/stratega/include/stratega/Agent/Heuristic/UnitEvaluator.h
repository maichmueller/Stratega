...
class UnitTypeEvaluator {
  public:
   /// <summary>
   /// Returns a dictionary mapping a unit - type to a type - strength.A higher strength value means
   /// the unit is more powerful. The evaluation uses the fact that the unit with the highest stats
   /// is the most powerful. Based on this, the score is a linear sum of various stats, divided by
   /// their maximum possible stat.
   ///
   /// As an example, if the highest maximum health a unit can have is 400, then all unit - types
   /// get a score of maximum_health / 400 added to their strength.
   /// </summary>
   ...
   static UnitTypeStrengthLookup computeLinearSumEvaluation(const GameState& gameState)
   {
      ...
      for(const auto& entry : gameState.getGameInfo()->getEntityTypes()) {
         for(const auto& parameter : entry.second.getParameters()) {
            ...
            if(parameter.second.getName() == "AttackRange"
               || parameter.second.getName() == "HealRange") {
      ...
      for(const auto& entry : gameState.getGameInfo()->getEntityTypes()) {
         ...
         for(const auto& parameter : entry.second.getParameters()) {
            ...
            if(parameter.second.getName() == "AttackRange"
               || parameter.second.getName() == "HealRange") {
               actionRangePerc =
                  (int) (parameter.second.getDefaultValue() / static_cast< double >(maxActionRange));
               continue;
            }
            if(parameter.second.getName() == "Health") {
