...
class AbstractHeuristic: public StateHeuristic {
  ...
  public:
   AbstractHeuristic(
      ...
      std::map< std::string, double >& attributeUValues,
      GameState& gameState)
       : attributeWeights(std::move(attributeWeights)),
         attributeUValues(std::move(attributeUValues))
   {
         ...
         for(const auto& entityType : gameState.getGameInfo()->getEntityTypes()) {
            for(const auto parameter : entityType.second.getParameters()) {
               if(parameter.second.getName() == parameterName
                  && parameter.second.getMaxValue() > maxValue[parameterName]) {
                  maxValue[parameterName] = parameter.second.getMaxValue();
               }
               if(parameter.second.getName() == parameterName
                  && parameter.second.getMinValue() < minValue[parameterName]) {
