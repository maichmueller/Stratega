...
struct RHEAParameters: AgentParameters {
  public:
   ...
   double
      mutationRate = 0.1;  // mutation rate when transferring one individual to the next generation
   ...
   bool continuePreviousSearch = true;  // initialize new population with shifted best individual of
                                        // the previous iteration
   size_t mutateBestN = 9;  // include Mutate_best additional copies of the shifted best individual
                            // in the next population

   double epsilon = 1e-2;  // the amount of noise for randomly modifying an individuals value
   std::uniform_real_distribution< double >
      doubleDistribution_ = std::uniform_real_distribution< double >(
         0, 1);  // Uniform distribution of real numbers in [0,1]
...
struct convert< SGA::RHEAParameters > {
   /// <summary>
   /// Reads Agent generic (via call rhs.decode(node)) and RHEA-specific parameters received in a
   /// YAML node. Add lines to this method to capture more parameters.
   /// </summary>
   /// <param name="node">YAML Node with the parameters for the agent.</param>
   /// <param name="rhs">Parameters object to be modified with the contents of the YAML
   /// node.</param> <returns>True if there was no problem.</returns>
   static bool decode(const Node& node, SGA::RHEAParameters& rhs)
   {
      ...
      rhs.continuePreviousSearch = node["ContPreviousSearch"].as< bool >(
         rhs.continuePreviousSearch);
