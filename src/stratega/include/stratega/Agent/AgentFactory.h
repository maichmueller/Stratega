  ...
  public:
   ...
   bool registerAgent(const std::string name)
   {
      return registerAgentFn(
         name, [](std::string name) { return static_cast< Agent* >(new T(name)); });
   }
   ...
   bool registerAgent(const std::string& name)
   {
      bool normalRegister = registerAgentFn(
         name, [](std::string name) { return static_cast< Agent* >(new T(name, Params())); });
      ...
      return registerAgentFn(name, [](std::string name, const YAML::Node node) {
         return static_cast< Agent* >(new T(name, node.as< Params >()));
      });
   }
