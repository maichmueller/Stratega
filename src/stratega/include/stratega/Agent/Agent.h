...
class Agent {
  public:
   ...
   /// <summary>
   /// Function for initializing the agent. Override this function to receive a call just before
   /// starts.
   /// </summary>
   ...
   /// <summary>
   /// Function for deciding the next action to execute. Must be overriden for an agent to work.
   /// Returns an ActionAssignment
   ...
   /// <returns></returns>
   virtual ActionAssignment computeAction(
      GameState state, const ForwardModel& forwardModel, Timer timer) = 0;
  ...
  protected:
   /// <summary>
   /// Returns the random generator of this player. <See cref="SGA::Agent.setSeed()"/> for how to
   /// set the seed of this agent.
   /// </summary>
   ...
   std::vector< Action > filterActionsByTypeID(
      const std::vector< Action >& actions, int typeID) const;
   ...
   /// <param name="typeID">Value of the parameter to use for the filter</param>
   /// <returns>A subset of actions (can be emtpy) that match the seach criteria.</returns>
   std::vector< Action > filterActionsByName(
      const std::vector< Action >& actions, std::string name) const;

   /// <summary>
   /// Filters a vector actions returning those actions of a particular Action Flag (e.g. EndTick,
   /// Continuous, Abort).
   /// </summary>
   /// <param name="actions">Larger vector of actions to filter from.</param>
   /// <param name="typeID">Value of the parameter to use for the filter</param>
   /// <returns>A subset of actions (can be emtpy) that match the seach criteria.</returns>
   std::vector< Action > filterActionsByFlag(
      const std::vector< Action >& actions, ActionFlag flag) const;

   /// <summary>
   /// Filters a vector actions returning those actions of a particular Source Type (e.g. Entity,
   /// Player).
   /// </summary>
   /// <param name="actions">Larger vector of actions to filter from.</param>
   /// <param name="typeID">Value of the parameter to use for the filter</param>
   /// <returns>A subset of actions (can be emtpy) that match the seach criteria.</returns>
   std::vector< Action > filterActionsBySource(
      const std::vector< Action >& actions, ActionSourceType source) const;

   /// <summary>
   /// Filters a vector actions returning those actions that would be executed by a particular
   /// entity, whose ID is provided.
   /// </summary>
   /// <param name="actions">Larger vector of actions to filter from.</param>
   /// <param name="typeID">Value of the parameter to use for the filter</param>
   /// <returns>A subset of actions (can be emtpy) that match the seach criteria.</returns>
   std::vector< Action > filterActionsByEntityID(
      const std::vector< Action >& actions, int entityID) const;

   /// <summary>
   /// Filters a vector actions returning those actions that would be executed by a particular
   /// player, whose ID is provided
   /// </summary>
   /// <param name="actions">Larger vector of actions to filter from.</param>
   /// <param name="typeID">Value of the parameter to use for the filter</param>
   /// <returns>A subset of actions (can be emtpy) that match the seach criteria.</returns>
   std::vector< Action > filterActionsByPlayerID(
      const std::vector< Action >& actions, int playerID) const;
};

/// <summary>
/// Given an opponent ID, it rolls the state forward executing an action dictated by the opponent
/// model from agParams. If no opponent model is set, nothing / skip turn is executed.
/// </summary>
...
inline bool rollOppAction(
   GameState& gs, const ForwardModel& fm, const AgentParameters& agParams, int oppID)
{
...
/// <summary>
/// Rolls the state forward for all OPPONENTS in the game. For all players with an ID !=
/// agParams.PLAYER_ID, the opponent model from agParams is used to decide the action. If no
/// opponent model is defined, nothing / skip turn is executed.
/// </summary>
/// <param name="gs">Game state to roll forward</param>
/// <param name="fm">Forward model to roll the state forward and quering available actions.</param>
/// <param name="agParams">Parameters of the agent who's calling this method.</param>
...
inline int rollOppOnly(GameState& gs, const ForwardModel& fm, const AgentParameters& agParams)
{
   ...
   for(int id : canPlay) {
      ...
      if(id != agParams.PLAYER_ID && rollOppAction(gs, fm, agParams, id))
         actionsExecuted++;
   }
...
/// <summary>
/// Rolls the state forward for all players in the game. For the player with an ID ==
/// agParams.PLAYER_ID, the action 'act' will be executed. For the rest, the opponent model from
/// agParams is used to decide the action. If no opponent model is defined, nothing / skip turn is
/// executed.
/// </summary>
/// <param name="gs">Game state to roll forward</param>
/// <param name="fm">Forward model to roll the state forward and quering available actions.</param>
...
inline int roll(
   ...
   const Action& act,
   int playerID,
   const AgentParameters& agParams)
{
   int actionsExecuted = 0;
