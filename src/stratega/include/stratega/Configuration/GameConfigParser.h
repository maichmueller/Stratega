...
class GameConfigParser {
  public:
   ...
   std::unordered_map< int, LevelDefinition > parseLevelsFromFile(
  ...
  private:
   std::unordered_set< EntityTypeID > parseEntityGroup(
      const YAML::Node& groupNode, const GameConfig& config) const;
   std::unordered_map< ParameterID, double > parseCost(
