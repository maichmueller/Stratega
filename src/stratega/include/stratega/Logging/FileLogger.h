...
class FileLogger {
  ...
  private:
   ...
   // Problem is that YAML::Node is a reference type, and that makes it much harder to write it
   // iteratively, since we can't use a single variable to represent our current node.
   static YAML::Node getNodeFromKey(
      const std::string& key, YAML::Node curNode, size_t keyPos, YAML::NodeType::value leafType)
   {
